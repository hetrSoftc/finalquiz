 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_overdue_attempt_updater {
        ...
        foreach ($attemptstoprocess as $attempt) {
            try {
                ...
                if (!$quiz || $attempt->quiz != $quiz->id) {
                    $quiz = $DB->get_record('finalquiz', array('id' => $attempt->quiz), '*', MUST_EXIST);
                    $cm = get_coursemodule_from_instance('finalquiz', $attempt->quiz);
                    $quizcount += 1;
                }
    ...
    /**
     * @return moodle_recordset of finalfinalquiz_attempts that need to be processed because time has
    ...
    public function get_list_of_overdue_attempts($processto) {
        ...
        $quizausersql = finalquiz_get_attempt_usertime_sql(
                "iquiza.state IN ('inprogress', 'overdue') AND iquiza.timecheckstate <= :iprocessto");

        // This query should have all the finalquiz_attempts columns.
