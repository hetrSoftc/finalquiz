...
require_once($CFG->dirroot . '/mod/finalquiz/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquiz_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the finalquiz settings object. */
    protected $quiz;

    /** @var context the finalquiz context. */
    protected $context;
     ...
     * @param object the finalquiz settings object.
     * @param context the finalquiz context.
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'finalquiz'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'finalquiz'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/finalquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'finalquiz', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'finalquiz'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/finalquiz:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/finalquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'finalquiz', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'finalquiz'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'finalquiz');
        $mform->setDefault('password', $this->quiz->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('quizopen', 'finalquiz'), mod_finalquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->quiz->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('quizclose', 'finalquiz'), mod_finalquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->quiz->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'finalquiz'), array('optional' => true));
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'finalquiz'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'finalquiz');
        $mform->setDefault('attempts', $this->quiz->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'finalquiz'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'finalquiz'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'finalquiz'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one final quiz setting was changed.
        $changed = false;
        ...
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'finalquiz');
        }
