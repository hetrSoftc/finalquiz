...
/**
 * Upgrade script for the finalquiz module.
 *
...
/**
 * finalQuiz module upgrade function.
 * @param string $oldversion the version we are upgrading from.
 */
function xmldb_finalquiz_upgrade($oldversion) {
    global $CFG, $DB;

    $dbman = $DB->get_manager();

    if ($oldversion < 2016092000) {
        // Define new fields to be added to finalquiz.
        $table = new xmldb_table('finalquiz');
        ...
        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2016092000, 'finalquiz');
    }

    if ($oldversion < 2016092001) {
        // New field for finalquiz_attemps.
        $table = new xmldb_table('finalfinalquiz_attempts');
        ...
        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2016092001, 'finalquiz');
    }

    if ($oldversion < 2016100300) {
        // Find finalquizzes with the combination of require passing grade and grade to pass 0.
        ...
        INNER JOIN {modules} m ON m.id = cm.module
             WHERE q.completionpass = 1
               ...
               AND gi.itemtype = ?
               AND m.name = ?", array('mod', 'finalquiz'));
        ...
        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2016100300, 'finalquiz');
    }
    ...
    if ($oldversion < 2018020700) {
        ...
        $table = new xmldb_table('finalquiz_slots');

        // Define field questioncategoryid to be added to finalquiz_slots.
        ...
        // Define key questioncategoryid (foreign) to be added to finalquiz_slots.
        ...
        // Define field includingsubcategories to be added to finalquiz_slots.
    ...
    if ($oldversion < 2018020701) {
        ...
        // For each question, move the configuration data to the finalquiz_slots table.
        $pbar = new progress_bar('updatefinalquizslotswithrandom', 500, true);
        foreach ($rs as $record) {
            $data = new stdClass();
            $data->id = $record->id;
            $data->questioncategoryid = $record->category;
            $data->includingsubcategories = empty($record->questiontext) ? 0 : 1;
            $DB->update_record('finalquiz_slots', $data);

            // Update progress.
            $a->done++;
            $pbar->update($a->done, $a->total, get_string('updatefinalquizslotswithrandomxofy', 'finalquiz', $a));
        }
        $rs->close();

        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2018020701, 'finalquiz');
    }

    if ($oldversion < 2018040700) {

        // Define field tags to be dropped from finalquiz_slots. This field was added earlier to master only.
        $table = new xmldb_table('finalquiz_slots');
        $field = new xmldb_field('tags');

        // Conditionally launch drop field finalquizid.
        if ($dbman->field_exists($table, $field)) {
            $dbman->drop_field($table, $field);
        }

        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040700, 'finalquiz');
    }

    if ($oldversion < 2018040800) {

        // Define table finalquiz_slot_tags to be created.
        $table = new xmldb_table('finalquiz_slot_tags');

        // Adding fields to table finalquiz_slot_tags.
        ...
        // Adding keys to table finalquiz_slot_tags.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->add_key('slotid', XMLDB_KEY_FOREIGN, array('slotid'), 'finalquiz_slots', array('id'));
        $table->add_key('tagid', XMLDB_KEY_FOREIGN, array('tagid'), 'tag', array('id'));

        // Conditionally launch create table for finalquiz_slot_tags.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // finalQuiz savepoint reached.
        upgrade_mod_savepoint(true, 2018040800, 'finalquiz');
    }
