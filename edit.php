...
list($thispageurl, $contexts, $cmid, $cm, $quiz, $pagevars) =
        question_edit_setup('editq', '/mod/finalquiz/edit.php', true);

$defaultcategoryobj = question_make_default_categories($contexts->all());
$defaultcategory = $defaultcategoryobj->id . ',' . $defaultcategoryobj->contextid;

$quizhasattempts = finalquiz_has_attempts($quiz->id);
...
// You need mod/finalquiz:manage in addition to question capabilities to access this page.
...
if (optional_param('repaginate', false, PARAM_BOOL) && confirm_sesskey()) {
    // Re-paginate the final quiz.
    $structure->check_can_be_edited();
    $questionsperpage = optional_param('questionsperpage', $quiz->questionsperpage, PARAM_INT);
    finalquiz_repaginate_questions($quiz->id, $questionsperpage );
    finalquiz_delete_previews($quiz);
    redirect($afteractionurl);
}

if (($addquestion = optional_param('addquestion', 0, PARAM_INT)) && confirm_sesskey()) {
    // Add a single question to the current final quiz.
    $structure->check_can_be_edited();
    finalquiz_require_question_use($addquestion);
    $addonpage = optional_param('addonpage', 0, PARAM_INT);
    finalquiz_add_finalquiz_question($addquestion, $quiz, $addonpage);
    finalquiz_delete_previews($quiz);
    finalquiz_update_sumgrades($quiz);
...
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    foreach ($rawdata as $key => $value) { // Parse input for question ids.
        if (preg_match('!^q([0-9]+)$!', $key, $matches)) {
            $key = $matches[1];
            finalquiz_require_question_use($key);
            finalquiz_add_finalquiz_question($key, $quiz, $addonpage);
        }
    }
    finalquiz_delete_previews($quiz);
    finalquiz_update_sumgrades($quiz);
...
if ($addsectionatpage = optional_param('addsectionatpage', false, PARAM_INT)) {
    // Add a section to the final quiz.
    $structure->check_can_be_edited();
...
if ((optional_param('addrandom', false, PARAM_BOOL)) && confirm_sesskey()) {
    // Add random questions to the final quiz.
    $structure->check_can_be_edited();
    $recurse = optional_param('recurse', 0, PARAM_BOOL);
    $addonpage = optional_param('addonpage', 0, PARAM_INT);
    $categoryid = required_param('categoryid', PARAM_INT);
    $randomcount = required_param('randomcount', PARAM_INT);
    finalquiz_add_random_questions($quiz, $addonpage, $categoryid, $randomcount, $recurse);
...
if (optional_param('savechanges', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (is_float($maxgrade) && $maxgrade >= 0) {
        finalquiz_set_grade($maxgrade, $quiz);
        finalquiz_update_all_final_grades($quiz);
        finalquiz_update_grades($quiz, 0, true);
    }
...
$questionbank->set_finalquiz_has_attempts($quizhasattempts);
$questionbank->process_actions($thispageurl, $cm);

// End of process commands =====================================================.

$PAGE->set_pagelayout('incourse');
$PAGE->set_pagetype('mod-finalquiz-edit');

$output = $PAGE->get_renderer('mod_finalquiz', 'edit');

$PAGE->set_title(get_string('editingfinalquizx', 'finalquiz', format_string($quiz->name)));
$PAGE->set_heading($course->fullname);
$node = $PAGE->settingsnav->find('mod_finalquiz_edit', navigation_node::TYPE_SETTING);
if ($node) {
    $node->make_active();
}
echo $OUTPUT->header();
...
$PAGE->requires->data_for_js('finalquiz_edit_config', $quizeditconfig);
$PAGE->requires->js('/question/qengine.js');

// Questions wrapper start.
echo html_writer::start_tag('div', array('class' => 'mod-finalquiz-edit-content'));
