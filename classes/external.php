...
class mod_finalquiz_external extends external_api {
    ...
    public static function get_finalquizzes_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $quizzes = get_all_instances_in_courses("finalquiz", $courses);
            foreach ($quizzes as $quiz) {
                $context = context_module::instance($quiz->coursemodule);

                // Update quiz with override information.
                $quiz = finalquiz_update_effective_access($quiz, $USER->id);
                ...
                if (has_capability('mod/finalquiz:view', $context)) {
                    // Format intro.
                    list($quizdetails['intro'], $quizdetails['introformat']) = external_format_text($quiz->intro,
                                                                    $quiz->introformat, $context->id, 'mod_finalquiz', 'intro', null);

                    $quizdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_finalquiz', 'intro', false, false);
                    ...
                    $hasfeedback = finalquiz_has_feedback($quiz);
                    $quizdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $quizobj = finalquiz::create($quiz->id, $USER->id);
                    $accessmanager = new finalquiz_access_manager($quizobj, $timenow, has_capability('mod/finalquiz:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the finalquiz.
                    if (!$accessmanager->prevent_access()) {
                        $quizdetails['hasquestions'] = (int) $quizobj->has_questions();
                        $quizdetails['autosaveperiod'] = get_config('finalquiz', 'autosaveperiod');
                ...
                'quizzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this final quiz is part of.'),
                            ...
                            'timeopen' => new external_value(PARAM_INT, 'The time when this final quiz opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this final quiz closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for final quiz attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a final quiz attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values FINALQUIZ_GRADEHIGHEST, FINALQUIZ_GRADEAVERAGE,
                                                                    FINALQUIZ_ATTEMPTFIRST or FINALQUIZ_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their final quiz
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_finalquiz_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their final quiz
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their final quiz attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            final quiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            final quiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their final quiz
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their final quiz
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the final quiz, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the final quiz. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the final quiz overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the final quiz was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a final quiz attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this final quiz can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark final quiz complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the final quiz to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the final quiz has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the final quiz has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a final quiz.
     *
     * @param int $quizid final quiz instance id
     * @return array array containing the final quiz, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_finalquiz($quizid) {
        global $DB;

        // Request and permission validation.
        $quiz = $DB->get_record('finalquiz', array('id' => $quizid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($quiz, 'finalquiz');
     ...
     * @since Moodle 3.1
     */
    public static function view_finalquiz_parameters() {
        return new external_function_parameters (
            array(
                'quizid' => new external_value(PARAM_INT, 'final quiz instance id'),
            )
        );
    }
     ...
     */
    public static function view_finalquiz($quizid) {
        global $DB;

        $params = self::validate_parameters(self::view_finalquiz_parameters(), array('quizid' => $quizid));
        $warnings = array();

        list($quiz, $course, $cm, $context) = self::validate_finalquiz($params['quizid']);

        // Trigger course_module_viewed event and completion.
        finalquiz_view($quiz, $course, $cm, $context);
    ...
    /**
     * Describes the view_finalquiz return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_finalquiz_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                'quizid' => new external_value(PARAM_INT, 'finalquiz instance id'),
                'userid' => new external_value(PARAM_INT, 'user id, empty for current user', VALUE_DEFAULT, 0),
                'status' => new external_value(PARAM_ALPHA, 'finalquiz status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
     ...
     *
     * @param int $quizid finalquiz instance id
     * @param int $userid user id
     * @param string $status finalquiz status: all, finished or unfinished
    ...
    public static function get_user_attempts($quizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/finalquiz:viewreports', $context);
        }

        $attempts = finalquiz_get_user_attempts($quiz->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'finalquiz' => new external_value(PARAM_INT, 'Foreign key reference to the finalquiz that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this finalquiz.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this finalquiz
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time finalquiz cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a finalquiz.
     *
     * @param int $quizid finalquiz instance id
     * @param int $userid user id
    ...
    public static function get_user_best_grade($quizid, $userid = 0) {
        ...
        $result = array();
        $grade = finalquiz_get_best_grade($quiz, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given final quiz.'),
    ...
    public static function get_combined_review_options($quizid, $userid = 0) {
        ...
        $attempts = finalquiz_get_user_attempts($quiz->id, $user->id, 'all', true);

        $result = array();
        $result['someoptions'] = [];
        $result['alloptions'] = [];

        list($someoptions, $alloptions) = finalquiz_get_combined_reviewoptions($quiz, $attempts);
    ...
    /**
     * Starts a new attempt at a finalquiz.
     *
     * @param int $quizid finalquiz instance id
    ...
    public static function start_attempt($quizid, $preflightdata = array(), $forcenew = false) {
        ...
        if (!$quizobj->has_questions()) {
            throw new moodle_finalquiz_exception($quizobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            finalquiz_validate_new_attempt($quizobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$quizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_finalquiz_exception($quizobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == finalquiz_attempt::OVERDUE) {
                    throw new moodle_finalquiz_exception($quizobj, 'stateoverdue');
                } else {
                    throw new moodle_finalquiz_exception($quizobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = finalquiz_prepare_and_start_new_attempt($quizobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
     ...
     * @param  bool $checkaccessrules whether to check the finalquiz access rules or not
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = finalquiz_attempt::create($params['attemptid']);
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == finalquiz_attempt::OVERDUE) {
            throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'stateoverdue');
        }
        ...
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'Out of sequence access');
            }
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the final quiz this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the final quiz', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the final quiz display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the final quiz display settings.'
        );
    }
     ...
     *
     * @param  finalquiz_attempt  $attemptobj  the final quiz attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(finalquiz_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_finalquizobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_finalquiz');
    ...
    /**
     * Returns information for the given attempt page for a finalquiz attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_finalquiz_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/finalquiz:preview capability,
    ...
    /**
     * Returns a summary of a finalquiz attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the finalquiz. This function is intended for the finalquiz auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a finalquiz and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_exception($attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new moodle_finalquiz_exception($attemptobj->get_finalquizobj(), 'noreviewattempt');
        }
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_finalquizobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = finalquiz_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_finalquiz(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'finalquiz'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the finalquiz (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
    ...
    /**
     * Describes the parameters for view_finalquiz.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_finalquiz_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'quizid' => new external_value(PARAM_INT, 'finalquiz instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given finalquiz.
     *
     * @param int $quizid quiz instance id
     ...
     * @throws moodle_exception
     */
    public static function get_finalquiz_feedback_for_grade($quizid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_finalquiz_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = finalquiz_feedback_record_for_grade($params['grade'], $quiz);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_finalquiz', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_finalquiz', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_finalquiz_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_finalquiz_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_finalquiz_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_finalquiz_access_information_parameters() {
        return new external_function_parameters (
            array(
                'quizid' => new external_value(PARAM_INT, 'finalquiz instance id')
            )
    ...
    /**
     * Return access information for a given finalquiz.
     *
     * @param int $quizid finalquiz instance id
     ...
     * @throws  moodle_finalquiz_exception
     */
    public static function get_finalquiz_access_information($quizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_finalquiz_access_information_parameters(), $params);

        list($quiz, $course, $cm, $context) = self::validate_finalquiz($params['quizid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/finalquiz:attempt', $context);;
        $result['canmanage'] = has_capability('mod/finalquiz:manage', $context);;
        $result['canpreview'] = has_capability('mod/finalquiz:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/finalquiz:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/finalquiz:viewreports', $context);;

        // Access manager now.
        $quizobj = finalquiz::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/finalquiz:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new finalquiz_access_manager($quizobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_finalquiz_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_finalquiz_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the finalquiz or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the finalquiz settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the finalquiz or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the finalquiz reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a finalquiz.
     *
     * @param int $quizid finalquiz instance id
    ...
    public static function get_attempt_access_information($quizid, $attemptid = 0) {
        ...
        $attempts = finalquiz_get_user_attempts($quiz->id, $USER->id, 'finished', true);
        $lastfinishedattempt = end($attempts);
        if ($unfinishedattempt = finalquiz_get_user_attempt_unfinished($quiz->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != finalquiz_attempt::IN_PROGRESS and $unfinishedattempt->state != finalquiz_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_finalquiz_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_finalquiz_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'quizid' => new external_value(PARAM_INT, 'finalquiz instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given finalquiz.
     ...
     * @throws  moodle_finalquiz_exception
     */
    public static function get_finalquiz_required_qtypes($quizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_finalquiz_required_qtypes_parameters(), $params);
    ...
    /**
     * Describes the get_finalquiz_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_finalquiz_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the finalquiz'),
                'warnings' => new external_warnings(),
            )
        );
    }
