...
namespace mod_finalquiz\output;
defined('MOODLE_INTERNAL') || die();

use \mod_finalquiz\structure;
use \html_writer;
use renderable;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \finalquiz $quizobj object containing all the quiz settings information.
     ...
     */
    public function edit_page(\finalquiz $quizobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingquizx', 'finalquiz',
                format_string($quizobj->get_finalquiz_name())), 'editingquiz', 'finalquiz', '',
                get_string('basicideasofquiz', 'finalquiz'), 2);

        // Information at the top.
        $output .= $this->finalquiz_state_warnings($structure);

        $output .= html_writer::start_div('mod_finalquiz-edit-top-controls');
        $output .= $this->finalquiz_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);

        $output .= html_writer::start_div('mod_finalquiz-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= $this->total_marks($quizobj->get_finalquiz());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_finalquiz/finalquizquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_finalquiz/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
     ...
     * @return string HTML to output.
     */
    public function finalquiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function finalquiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'finalquiz', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_finalquiz_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary m-l-1',
                'name' => 'savechanges', 'value' => get_string('save', 'finalquiz')));
    ...
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        ...
        $header = html_writer::tag('span', get_string('repaginatecommand', 'finalquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'finalquiz'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->yui_module('moodle-mod_finalquiz-repaginate', 'M.mod_finalquiz.repaginate.init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'finalquiz'),
            'class' => 'btn btn-secondary m-b-1'
        );
        if (!$structure->can_be_edited()) {
            $buttonoptions['disabled'] = 'disabled';
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'finalquiz'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_finalquiz'),
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_finalquiz'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            'class' => 'btn-toolbar',
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'finalquiz'),
        );
        ...
        $output .= html_writer::tag('div',
                html_writer::tag('div',
                        html_writer::link('#', get_string('selectall', 'finalquiz'), $buttonselectalloptions) .
                        html_writer::tag('span', "/", ['class' => 'separator']) .
                        html_writer::link('#', get_string('selectnone', 'finalquiz'), $buttondeselectalloptions),
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'finalquiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'finalquiz', $select) .
    ...
    public function total_marks($quiz) {
        $totalmark = html_writer::span(finalquiz_format_grade($quiz, $quiz->sumgrades), 'mod_finalquiz_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'finalquiz', $totalmark),
     ...
     *
     * @param structure $structure the structure of the finalquiz being edited.
     * @param \stdClass $section The finalquiz_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'finalquiz', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the finalquiz.
     * @param \stdClass $section data from the finalquiz_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'finalquiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'finalquiz'),
    ...
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'finalquiz', $section->heading);
        $url = new \moodle_url('/mod/finalquiz/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the finalquiz.
     *
     * This function calls {@link core_course_renderer::finalquiz_section_question()}
     *
     * @param structure $structure object containing the structure of the finalquiz.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_finalquiz($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-finalquiz-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'finalquiz'), array('class' => 'add-menu'));
        ...
        // Disable the link if finalquiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'finalquiz');
        }
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'finalquiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'finalquiz');
        }
        ...
        $returnurl = new \moodle_url('/mod/finalquiz/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/finalquiz/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'finalquiz', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'finalquiz');
        }
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_finalquiz(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $quiz the finalquiz settings from the database.
     * @param \stdClass $question data from the question and finalquiz_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($quiz, $question, $label = null, $variant = null) {
        $url = finalquiz_question_preview_url($quiz, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'finalquiz');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'finalquiz');
    ...
    /**
     * Render an icon to remove a question from the finalquiz.
     *
     * @param structure $structure object containing the structure of the finalquiz.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the finalquiz.
     *
     * @param structure $structure object containing the structure of the finalquiz.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('quizid' => $structure->get_finalquizid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'finalquiz');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'finalquiz');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'finalquiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'finalquiz'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'finalquiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'finalquiz'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a finalquiz edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = finalquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'finalquiz').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/finalquiz/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = finalquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'finalquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'finalquiz'), array('class' => 'mod_finalquiz_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'finalquiz')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'finalquiz'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_finalquiz/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_finalquiz\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/finalquiz/edit_rest.php';
        $config->sectionurl = '/mod/finalquiz/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_finalquiz-toolboxes',
                'M.mod_finalquiz.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'quizid' => $structure->get_finalquizid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'finalquiz');
        $this->page->requires->yui_module('moodle-mod_finalquiz-toolboxes',
                'M.mod_finalquiz.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'quizid' => $structure->get_finalquizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_finalquiz-dragdrop', 'M.mod_finalquiz.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizid' => $structure->get_finalquizid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_finalquiz-dragdrop', 'M.mod_finalquiz.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizid' => $structure->get_finalquizid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'finalquiz', '');
     ...
     *
     * @param \mod_finalquiz\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_finalquiz\question\bank\custom_view $questionbank, array $pagevars) {
