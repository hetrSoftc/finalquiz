...
/**
 * Defines the \mod_finalquiz\structure class.
 *
...
namespace mod_finalquiz;
defined('MOODLE_INTERNAL') || die();

/**
 * Quiz structure class.
 *
 * The structure of the finalquiz. That is, which questions it is built up
 * from. This is used on the Edit finalquiz page (edit.php) and also when
 * starting an attempt at the finalquiz (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \finalquiz the finalquiz this is the structure of. */
    protected $quizobj = null;

    /**
     * @var \stdClass[] the questions in this quiz. Contains the row from the questions
     * table, with the data from the finalquiz_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] finalquiz_slots.id => the finalquiz_slots rows for this finalquiz, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] finalquiz_slots.slot => the finalquiz_slots rows for this finalquiz, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * finalquiz_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty finalquiz.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given finalquiz.
     * @param \finalquiz $quizobj the finalquiz.
     * @return structure
     */
    public static function create_for_finalquiz($quizobj) {
        $structure = self::create();
        $structure->quizobj = $quizobj;
        $structure->populate_structure($quizobj->get_finalquiz());
        return $structure;
    }

    /**
     * Whether there are any questions in the finalquiz.
     * @return bool true if there is at least one question in the finalquiz.
     */
    ...
    /**
     * Get the number of questions in the finalquiz.
     * @return int the number of questions in the finalquiz.
     */
     ...
     * question_category.contextid, and the finalquiz_slots data for the question in this finalquiz.
     */
     ...
     * questions cannot be depended upon because of finalquiz options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->quizobj->get_navigation_method() == FINALQUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->quizobj->get_finalquiz()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_finalquiz', $this->quizobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->quizobj->get_finalquiz()->preferredbehaviour);
    ...
    /**
     * Get the course id that the finalquiz belongs to.
     * @return int the course.id for the finalquiz.
     */
    ...
    /**
     * Get the course module id of the finalquiz.
     * @return int the course_modules.id for the finalquiz.
     */
    ...
    /**
     * Get id of the finalquiz.
     * @return int the finalquiz.id for the finalquiz.
     */
    ...
    /**
     * Get the finalquiz object.
     * @return \stdClass the finalquiz settings row from the database.
     */
     ...
     * @return bool whether this finalquiz can be repaginated.
     */
     ...
     * @return bool whether the finalquiz can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !finalquiz_has_attempts($this->quizobj->get_finalquizid());
        }
        return $this->canbeedited;
    }

    /**
     * This finalquiz can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = finalquiz_attempt_summary_link_to_reports($this->get_finalquiz(),
                    $this->quizobj->get_cm(), $this->quizobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'finalquiz',
                    new \moodle_url('/mod/finalquiz/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the finalquiz.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the finalquiz.
     * @return int the number of questions that should be on each page of the
     * finalquiz by default.
     */
    public function get_questions_per_page() {
        return $this->quizobj->get_finalquiz()->questionsperpage;
    }

    /**
     * Get final quiz slots.
     * @return \stdClass[] the slots in this finalquiz.
     */
     ...
     */
    public function is_last_slot_in_finalquiz($slotnumber) {
     ...
     * @return \stdClass the finalquiz_slots for for the final slot in the quiz.
     */
     ...
     * @return \stdClass the requested finalquiz_slots row.
     */
     ...
     */
    public function formatted_finalquiz_grade() {
        return finalquiz_format_grade($this->get_finalquiz(), $this->get_finalquiz()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return finalquiz_format_question_grade($this->get_finalquiz(), $this->slotsinorder[$slotnumber]->maxmark);
    }
    ...
    public function get_decimal_places_for_grades() {
        return $this->get_finalquiz()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return finalquiz_get_grade_format($this->get_finalquiz());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (finalquiz_has_attempts($this->quizobj->get_finalquizid())) {
            $reviewlink = finalquiz_attempt_summary_link_to_reports($this->quizobj->get_finalquiz(),
                    $this->quizobj->get_cm(), $this->quizobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'finalquiz', $reviewlink);
        }
    ...
    public function get_dates_summary() {
        $timenow = time();
        $quiz = $this->quizobj->get_finalquiz();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($quiz->timeopen > 0) {
            if ($timenow > $quiz->timeopen) {
                $dates[] = get_string('quizopenedon', 'finalquiz', userdate($quiz->timeopen));
            } else {
                $dates[] = get_string('quizwillopen', 'finalquiz', userdate($quiz->timeopen));
            }
        }
        if ($quiz->timeclose > 0) {
            if ($timenow > $quiz->timeclose) {
                $dates[] = get_string('quizclosed', 'finalquiz', userdate($quiz->timeclose));
            } else {
                $dates[] = get_string('quizcloseson', 'finalquiz', userdate($quiz->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'finalquiz');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $quiz->timeopen) {
            $currentstatus = get_string('quizisclosedwillopen', 'finalquiz',
                    userdate($quiz->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($quiz->timeclose && $timenow <= $quiz->timeclose) {
            $currentstatus = get_string('quizisopenwillclose', 'finalquiz',
                    userdate($quiz->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($quiz->timeclose && $timenow > $quiz->timeclose) {
            $currentstatus = get_string('quizisclosed', 'finalquiz');
        } else {
            $currentstatus = get_string('quizisopen', 'finalquiz');
        }
    ...
    public function populate_structure($quiz) {
        ...
        $this->sections = $DB->get_records('finalquiz_sections', array('quizid' => $quiz->id), 'firstslot ASC');
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'finalquiz');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'finalquiz');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_finalquiz($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_finalquiz($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_finalquiz($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('finalquiz_slots', 'slot', $slotreorder,
                    array('quizid' => $this->get_finalquizid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('finalquiz_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        finalquiz_update_section_firstslots($this->get_finalquizid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE quizid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {finalquiz_slots} WHERE quizid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_finalquizid(), $this->get_finalquizid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {finalquiz_slots}
                       SET page = page - 1
                     WHERE quizid = ?
                       AND page > ?
                    ", array($this->get_finalquizid(), $page));
        }
    ...
    /**
     * Refresh page numbering of finalquiz slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('finalquiz_slots', array('quizid' => $this->get_finalquizid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of finalquiz slots and save to the database.
     * @param \stdClass $quiz the finalquiz object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('finalquiz_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    /**
     * Remove a slot from a finalquiz
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('finalquiz_slots', array('quizid' => $this->get_finalquizid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {finalquiz_slots} WHERE quizid = ?', array($this->get_finalquizid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('finalquiz_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('finalquiz_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('finalquiz_slots', 'slot', $i - 1,
                    array('quizid' => $this->get_finalquizid(), 'slot' => $i));
        }
        ...
        finalquiz_update_section_firstslots($this->get_finalquizid(), -1, $slotnumber);
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('finalquiz_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_finalquiz($slot->quizid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('finalquiz_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the finalquiz_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $quizslots = $DB->get_records('finalquiz_slots', array('quizid' => $this->get_finalquizid()), 'slot');
        $repaginate = new \mod_finalquiz\repaginate($this->get_finalquizid(), $quizslots);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'finalquiz');
        }
        $section->quizid = $this->get_finalquizid();
        $slotsonpage = $DB->get_records('finalquiz_slots', array('quizid' => $this->get_finalquizid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('finalquiz_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('finalquiz_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('finalquiz_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('finalquiz_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a finalquiz.');
        }
        $DB->delete_records('finalquiz_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = finalquiz_retrieve_tags_for_slot_ids($slotids);
    }
