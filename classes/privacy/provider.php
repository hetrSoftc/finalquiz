...
/**
 * Privacy Subsystem implementation for mod_finalquiz.
 *
...
namespace mod_finalquiz\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $items) : collection {
        // The table 'finalquiz' stores a record for each quiz.
        // It does not contain user personal data, but data is returned from it for contextual requirements.

        // The table 'finalquiz_attempts' stores a record of each quiz attempt.
        // It contains a userid which links to the user making the attempt and contains information about that attempt.
        $items->add_database_table('finalquiz_attempts', [
                'attempt'               => 'privacy:metadata:finalquiz_attempts:attempt',
                'currentpage'           => 'privacy:metadata:finalquiz_attempts:currentpage',
                'preview'               => 'privacy:metadata:finalquiz_attempts:preview',
                'state'                 => 'privacy:metadata:finalquiz_attempts:state',
                'timestart'             => 'privacy:metadata:finalquiz_attempts:timestart',
                'timefinish'            => 'privacy:metadata:finalquiz_attempts:timefinish',
                'timemodified'          => 'privacy:metadata:finalquiz_attempts:timemodified',
                'timemodifiedoffline'   => 'privacy:metadata:finalquiz_attempts:timemodifiedoffline',
                'timecheckstate'        => 'privacy:metadata:finalquiz_attempts:timecheckstate',
                'sumgrades'             => 'privacy:metadata:finalquiz_attempts:sumgrades',
            ], 'privacy:metadata:finalquiz_attempts');
        ...
        $items->add_database_table('quiz_grades', [
                'finalquiz'                  => 'privacy:metadata:quiz_grades:quiz',
        ...
        $items->add_database_table('quiz_overrides', [
                'finalquiz'                  => 'privacy:metadata:quiz_overrides:quiz',
        ...
        $items->add_plugintype_link('finalquiz', [], 'privacy:metadata:quiz');
        $items->add_plugintype_link('finalquizaccess', [], 'privacy:metadata:quizaccess');
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $qubaid = \core_question\privacy\provider::get_related_question_usages_for_user('rel', 'mod_finalquiz', 'qa.uniqueid', $userid);
        ...
        $sql = "SELECT c.id
                  ...
                  JOIN {finalquiz_attempts} qa ON qa.quiz = q.id
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        foreach ($quizzes as $quiz) {
            ...
            $components = \core_component::get_plugin_list('finalquizaccess');
            $exportparams = [
                    $quizobj,
                    $user,
                ];
        ...
        static::export_finalfinalquiz_attempts($contextlist);
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $cm = get_coursemodule_from_id('finalquiz', $context->instanceid);
        if (!$cm) {
            // Only quiz module will be handled.
            return;
        }
     ...
     */
    protected static function export_finalfinalquiz_attempts(approved_contextlist $contextlist) {
        global $DB;
        ...
        $sql = "SELECT
                  ...
                  FROM {context} c
                  JOIN {course_modules} cm ON cm.id = c.instanceid AND c.contextlevel = :contextlevel
                  JOIN {modules} m ON m.id = cm.module AND m.name = 'finalquiz'
                  JOIN {quiz} q ON q.id = cm.instance
                  JOIN {finalquiz_attempts} qa ON qa.quiz = q.id
            " . $qubaid->from. "
            WHERE (
        ...
        foreach ($attempts as $attempt) {
            $quiz = $DB->get_record('finalquiz', ['id' => $attempt->quiz]);
            $context = \context_module::instance($attempt->cmid);
            $attemptsubcontext = helper::get_finalquiz_attempt_subcontext($attempt, $contextlist->get_user());
