...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('finalquiz_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'finalquiz');
    }
    if (! $quiz = $DB->get_record('finalquiz', array('id' => $override->quiz))) {
...
$url = new moodle_url('/mod/finalquiz/overrideedit.php');
if ($action) {
...
$overridelisturl = new moodle_url('/mod/finalquiz/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new finalquiz_override_form($url, $cm, $quiz, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('finalquiz_overrides', $conditions)) {
            ...
            $quiz->cmid = $cm->id;
            finalquiz_delete_override($quiz, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('finalquiz_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('finalquiz_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        finalquiz_update_events($quiz, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'finalquiz');
