...
$PAGE->set_url('/mod/finalquiz/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'finalquiz');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/finalquiz:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'finalquiz', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'finalquiz' => format_string($attemptobj->get_finalquiz_name()), 'user' => fullname($student))));
...
$summarydata['quizname'] = array(
    'title'   => get_string('modulename', 'finalquiz'),
    'content' => format_string($attemptobj->get_finalquiz_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'finalquiz'),
...
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_finalquiz.init_comment_popup', null, false, finalquiz_get_js_module());
