...
$PAGE->set_url('/mod/finalquiz/edit-rest.php',
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'finalquiz_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/finalquiz:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/finalquiz:manage', $modcontext);
                        $slot = $DB->get_record('finalquiz_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => finalquiz_format_question_grade($quiz, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        if ($structure->update_slot_maxmark($slot, $maxmark)) {
                            ...
                            finalquiz_update_all_attempt_sumgrades($quiz);
                            finalquiz_update_all_final_grades($quiz);
                            finalquiz_update_grades($quiz, 0, true);
                        }
                        $result = array('instancemaxmark' => finalquiz_format_question_grade($quiz, $maxmark),
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('finalquiz_slots', array('quizid' => $quiz->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (finalquiz_has_question_use($quiz, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => finalquiz_format_grade($quiz, $quiz->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/finalquiz:manage', $modcontext);
                if (!$slot = $DB->get_record('finalquiz_slots', array('quizid' => $quiz->id, 'id' => $id))) {
