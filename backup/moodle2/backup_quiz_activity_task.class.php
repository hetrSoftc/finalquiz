...
/**
 * Defines backup_finalquiz_activity_task class
 *
...
require_once($CFG->dirroot . '/mod/finalquiz/backup/moodle2/backup_finalquiz_stepslib.php');

/**
 * Provides the steps to perform one complete backup of the final Quiz instance
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_finalquiz_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        ...
        $this->add_step(new backup_finalquiz_activity_structure_step('finalquiz_structure', 'finalquiz.xml'));

        // Note: Following  steps must be present
        // in all the activities using question banks (only finalquiz for now)
    ...
    public static function encode_content_links($content) {
        ...
        $search="/(".$base."\/mod\/finalquiz\/index.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZINDEX*$2@$', $content);

        // Link to final quiz view by moduleid.
        $search="/(".$base."\/mod\/finalquiz\/view.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@QUIZVIEWBYID*$2@$', $content);

        // Link to final quiz view by quizid.
        $search="/(".$base."\/mod\/finalquiz\/view.php\?q\=)([0-9]+)/";
