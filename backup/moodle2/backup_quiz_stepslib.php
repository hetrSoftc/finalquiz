...
/**
 * Define all the backup steps that will be used by the backup_finalquiz_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_finalquiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $quiz = new backup_nested_element('finalquiz', array('id'), array(
        ...
        $this->add_subplugin_structure('finalquizaccess', $quiz, true);
        ...
        $this->add_subplugin_structure('finalquizaccess', $attempt, true);
        ...
        $section->set_source_table('finalquiz_sections',
                array('quizid' => backup::VAR_PARENTID));

        $feedback->set_source_table('finalquiz_feedback',
                array('quizid' => backup::VAR_PARENTID));

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = array('finalquiz' => backup::VAR_PARENTID);
        if (!$userinfo) { //  Without userinfo, skip user overrides.
            $overrideparams['userid'] = backup_helper::is_sqlparam(null);

        }
        $override->set_source_table('finalquiz_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('finalquiz_grades', array('finalquiz' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {finalfinalquiz_attempts}
                    WHERE quiz = :quiz AND preview = 0',
                    array('finalquiz' => backup::VAR_PARENTID));
        }
        ...
        $quiz->annotate_files('mod_finalquiz', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_finalquiz', 'feedback', 'id');

        // Return the root element (finalquiz), wrapped into standard activity structure.
