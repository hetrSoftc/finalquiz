...
require_once($CFG->dirroot . '/mod/finalquiz/accessmanager.php');
require_once($CFG->dirroot . '/mod/finalquiz/accessmanager_form.php');
require_once($CFG->dirroot . '/mod/finalquiz/renderer.php');
require_once($CFG->dirroot . '/mod/finalquiz/attemptlib.php');
 ...
 */
define('FINALQUIZ_SHOW_TIME_BEFORE_DEADLINE', '3600');
 ...
 */
function finalquiz_create_attempt(quiz $quizobj, $attemptnumber, $lastattempt, $timenow, $ispreview = false, $userid = null) {
    global $USER;
    ...
    if ($attemptnumber == 1 || !$quiz->attemptonlast) {
        ...
        if (empty($lastattempt)) {
            print_error('cannotfindprevattempt', 'finalquiz');
        }
        $attempt = $lastattempt;
    }
 ...
 *
 * @param finalquiz      $quizobj            the quiz object to start an attempt for.
 ...
 */
function finalquiz_start_new_attempt($quizobj, $quba, $attempt, $attemptnumber, $timenow,
    ...
    if ($randomfound) {
        ...
        foreach ($quizobj->get_questions() as $questiondata) {
            ...
            $tagids = finalquiz_retrieve_slot_tag_ids($questiondata->slotid);
            ...
            if (isset($questionids[$quba->next_slot_number()])) {
                if ($randomloader->is_question_available($questiondata->category,
                    ...
                    $questions[$slot] = question_bank::load_question(
                            $questionids[$quba->next_slot_number()], $quizobj->get_finalquiz()->shuffleanswers);
                    continue;
                } else {
            ...
            if ($questionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'finalquiz',
                                           $quizobj->view_url(), $questiondata);
            }

            $questions[$slot] = question_bank::load_question($questionid,
                    $quizobj->get_finalquiz()->shuffleanswers);
        }
    }
    ...
    foreach ($sections as $section) {
        if ($section->shufflequestions) {
            ...
            foreach ($questionsinthissection as $slot) {
                if ($questionsonthispage && $questionsonthispage == $quizobj->get_finalquiz()->questionsperpage) {
                    $layout[] = 0;
                    $questionsonthispage = 0;
                }
                $layout[] = $slot;
 ...
 *
 */
function finalquiz_start_attempt_built_on_last($quba, $attempt, $lastattempt) {
 ...
 *
 * @param finalquiz                       $quizobj
 * @param question_usage_by_activity $quba
 * @param object                     $attempt
 * @return object                    attempt object with uniqueid and id set.
 */
function finalquiz_attempt_save_started($quizobj, $quba, $attempt) {
    global $DB;
    // Save the attempt in the database.
    question_engine::save_questions_usage_by_activity($quba);
    $attempt->uniqueid = $quba->get_id();
    $attempt->id = $DB->insert_record('finalfinalquiz_attempts', $attempt);
    ...
    if ($attempt->preview) {
        ...
        $event = \mod_finalquiz\event\attempt_preview_started::create($params);
    } else {
        $event = \mod_finalquiz\event\attempt_started::create($params);

    }

    // Trigger the event.
    $event->add_record_snapshot('finalquiz', $quizobj->get_finalquiz());
    $event->add_record_snapshot('finalfinalquiz_attempts', $attempt);
    $event->trigger();
 ...
 */
function finalquiz_get_user_attempt_unfinished($quizid, $userid) {
    $attempts = finalquiz_get_user_attempts($quizid, $userid, 'unfinished', true);
    if ($attempts) {
...
/**
 * Delete a finalquiz attempt.
 * @param mixed $attempt an integer attempt id or an attempt object
 *      (row of the finalfinalquiz_attempts table).
 * @param object $finalquiz the finalquiz object.
 */
function finalquiz_delete_attempt($attempt, $quiz) {
    global $DB;
    if (is_numeric($attempt)) {
        if (!$attempt = $DB->get_record('finalfinalquiz_attempts', array('id' => $attempt))) {
            return;
        }
    }

    if ($attempt->quiz != $quiz->id) {
        debugging("Trying to delete attempt $attempt->id which belongs to final quiz $attempt->quiz " .
                "but was passed final quiz $quiz->id.");
        return;
    }

    if (!isset($quiz->cmid)) {
        $cm = get_coursemodule_from_instance('finalquiz', $quiz->id, $quiz->course);
        $quiz->cmid = $cm->id;
    }

    question_engine::delete_questions_usage_by_activity($attempt->uniqueid);
    $DB->delete_records('finalfinalquiz_attempts', array('id' => $attempt->id));
    ...
    if (!$attempt->preview) {
        ...
        $event = \mod_finalquiz\event\attempt_deleted::create($params);
        $event->add_record_snapshot('finalfinalquiz_attempts', $attempt);
        $event->trigger();
    }

    // Search finalfinalquiz_attempts for other instances by this user.
    // If none, then delete record for this final quiz, this user from finalquiz_grades
    // else recalculate best grade.
    $userid = $attempt->userid;
    if (!$DB->record_exists('finalfinalquiz_attempts', array('userid' => $userid, 'finalquiz' => $quiz->id))) {
        $DB->delete_records('finalquiz_grades', array('userid' => $userid, 'finalquiz' => $quiz->id));
    } else {
        finalquiz_save_best_grade($quiz, $userid);
    }

    finalquiz_update_grades($quiz, $userid);
}

/**
 * Delete all the preview attempts at a final quiz, or possibly all the attempts belonging
 * to one user.
 * @param object $quiz the final quiz object.
 * @param int $userid (optional) if given, only delete the previews belonging to this user.
 */
function finalquiz_delete_previews($quiz, $userid = null) {
    global $DB;
    $conditions = array('finalquiz' => $quiz->id, 'preview' => 1);
    if (!empty($userid)) {
        $conditions['userid'] = $userid;
    }
    $previewattempts = $DB->get_records('finalfinalquiz_attempts', $conditions);
    foreach ($previewattempts as $attempt) {
        finalquiz_delete_attempt($attempt, $quiz);
    }
}

/**
 * @param int $quizid The final quiz id.
 * @return bool whether this final quiz has any (non-preview) attempts.
 */
function finalquiz_has_attempts($quizid) {
    global $DB;
    return $DB->record_exists('finalfinalquiz_attempts', array('finalquiz' => $quizid, 'preview' => 0));
}

// Functions to do with final quiz layout and pages //////////////////////////////////

/**
 * Repaginate the questions in a final quiz
 * @param int $quizid the id of the final quiz to repaginate.
 * @param int $slotsperpage number of items to put on each page. 0 means unlimited.
 */
function finalquiz_repaginate_questions($quizid, $slotsperpage) {
    global $DB;
    $trans = $DB->start_delegated_transaction();

    $sections = $DB->get_records('finalquiz_sections', array('quizid' => $quizid), 'firstslot ASC');
    $firstslots = array();
    foreach ($sections as $section) {
    ...
    $slots = $DB->get_records('finalquiz_slots', array('quizid' => $quizid),
            'slot');
    $currentpage = 1;
    $slotsonthispage = 0;
    foreach ($slots as $slot) {
        ...
        if ($slot->page != $currentpage) {
            $DB->set_field('finalquiz_slots', 'page', $currentpage, array('id' => $slot->id));
        }
        $slotsonthispage += 1;
    }
...
// Functions to do with final quiz grades ////////////////////////////////////////////

/**
 * Convert the raw grade stored in $attempt into a grade out of the maximum
 * grade for this final quiz.
 *
 * @param float $rawgrade the unadjusted grade, fof example $attempt->sumgrades
 * @param object $quiz the final quiz object. Only the fields grade, sumgrades and decimalpoints are used.
 ...
 */
function finalquiz_rescale_grade($rawgrade, $quiz, $format = true) {
    if (is_null($rawgrade)) {
        $grade = null;
    ...
    if ($format === 'question') {
        $grade = finalquiz_format_question_grade($quiz, $grade);
    } else if ($format) {
        $grade = finalquiz_format_grade($quiz, $grade);
    }
    return $grade;
}

/**
 * Get the feedback object for this grade on this final quiz.
 *
 * @param float $grade a grade on this final quiz.
 * @param object $quiz the final quiz settings.
 * @return false|stdClass the record object or false if there is not feedback for the given grade
 * @since  Moodle 3.1
 */
function finalquiz_feedback_record_for_grade($grade, $quiz) {
    global $DB;
    ...
    $feedback = $DB->get_record_select('finalquiz_feedback',
 ...
 * got this grade on this final quiz. The feedback is processed ready for diplay.
 *
 * @param float $grade a grade on this final quiz.
 * @param object $quiz the final quiz settings.
 * @param object $context the final quiz context.
 * @return string the comment that corresponds to this grade (empty string if there is not one.
 */
function finalquiz_feedback_for_grade($grade, $quiz, $context) {

    if (is_null($grade)) {
        return '';
    }

    $feedback = finalquiz_feedback_record_for_grade($grade, $quiz);
    ...
    $feedbacktext = file_rewrite_pluginfile_urls($feedback->feedbacktext, 'pluginfile.php',
            $context->id, 'mod_finalquiz', 'feedback', $feedback->id);
...
/**
 * @param object $quiz the final quiz database row.
 * @return bool Whether this final quiz has any non-blank feedback text.
 */
function finalquiz_has_feedback($quiz) {
    global $DB;
    static $cache = array();
    if (!array_key_exists($quiz->id, $cache)) {
        $cache[$quiz->id] = finalquiz_has_grades($quiz) &&
                $DB->record_exists_select('finalquiz_feedback', "quizid = ? AND " .
                    $DB->sql_isnotempty('finalquiz_feedback', 'feedbacktext', false, true),
                array($quiz->id));
    }
...
/**
 * Update the sumgrades field of the final quiz. This needs to be called whenever
 * the grading structure of the final quiz is changed. For example if a question is
 * added or removed, or a question weight is changed.
 *
 * You should call {@link finalquiz_delete_previews()} before you call this function.
 *
 * @param object $quiz a final quiz.
 */
function finalquiz_update_sumgrades($quiz) {
    global $DB;

    $sql = 'UPDATE {finalquiz}
            SET sumgrades = COALESCE((
                SELECT SUM(maxmark)
                FROM {finalquiz_slots}
                WHERE quizid = {finalquiz}.id
            ), 0)
            WHERE id = ?';
    $DB->execute($sql, array($quiz->id));
    $quiz->sumgrades = $DB->get_field('finalquiz', 'sumgrades', array('id' => $quiz->id));

    if ($quiz->sumgrades < 0.000005 && finalquiz_has_attempts($quiz->id)) {
        // If the final quiz has been attempted, and the sumgrades has been
        // set to 0, then we must also set the maximum possible grade to 0, or
        // we will get a divide by zero error.
        finalquiz_set_grade(0, $quiz);
    }
}

/**
 * Update the sumgrades field of the attempts at a final quiz.
 *
 * @param object $quiz a final quiz.
 */
function finalquiz_update_all_attempt_sumgrades($quiz) {
    global $DB;
    $dm = new question_engine_data_mapper();
    $timenow = time();

    $sql = "UPDATE {finalfinalquiz_attempts}
            SET
                timemodified = :timenow,
                sumgrades = (
                    {$dm->sum_usage_marks_subquery('uniqueid')}
                )
            WHERE finalquiz = :quizid AND state = :finishedstate";
    $DB->execute($sql, array('timenow' => $timenow, 'quizid' => $quiz->id,
            'finishedstate' => finalquiz_attempt::FINISHED));
}
 ...
 */
function finalquiz_set_grade($newgrade, $quiz) {
    global $DB;
    ...
    $DB->set_field('finalquiz', 'grade', $newgrade, array('id' => $quiz->instance));
    ...
    if ($oldgrade < 1) {
        ...
        finalquiz_update_all_final_grades($quiz);

    } else {
        // We can rescale the grades efficiently.
        $timemodified = time();
        $DB->execute("
                UPDATE {finalquiz_grades}
    ...
    finalquiz_grade_item_update($quiz);
    finalquiz_update_grades($quiz);
 ...
 */
function finalquiz_save_best_grade($quiz, $userid = null, $attempts = array()) {
    ...
    if (!$attempts) {
        ...
        $attempts = finalquiz_get_user_attempts($quiz->id, $userid);
    }

    // Calculate the best grade.
    $bestgrade = finalquiz_calculate_best_grade($quiz, $attempts);
    $bestgrade = finalquiz_rescale_grade($bestgrade, $quiz, false);

    // Save the best grade in the database.
    if (is_null($bestgrade)) {
        $DB->delete_records('finalquiz_grades', array('finalquiz' => $quiz->id, 'userid' => $userid));

    } else if ($grade = $DB->get_record('finalquiz_grades',
            array('finalquiz' => $quiz->id, 'userid' => $userid))) {
        $grade->grade = $bestgrade;
        $grade->timemodified = time();
        $DB->update_record('finalquiz_grades', $grade);
        ...
        $grade->grade = $bestgrade;
        $grade->timemodified = time();
        $DB->insert_record('finalquiz_grades', $grade);
    }
 ...
 *
 * @param object $quiz the final quiz settings object.
 * @param array $attempts an array of all the user's attempts at this final quiz in order.
 * @return float          the overall grade
 */
function finalquiz_calculate_best_grade($quiz, $attempts) {

    switch ($quiz->grademethod) {

        case FINALQUIZ_ATTEMPTFIRST:
            $firstattempt = reset($attempts);
            return $firstattempt->sumgrades;

        case FINALQUIZ_ATTEMPTLAST:
            $lastattempt = end($attempts);
            return $lastattempt->sumgrades;

        case FINALQUIZ_GRADEAVERAGE:
            $sum = 0;
            $count = 0;
 ...
 * @param object $quiz the final quiz settings.
 */
function finalquiz_update_all_final_grades($quiz) {
    global $DB;
    ...
    $firstlastattemptjoin = "JOIN (
            ...
            FROM {finalfinalquiz_attempts} iquiza
    ...
    $finalgradesubquery = "
            ...
            FROM {finalfinalquiz_attempts} quiza
            $join
            WHERE
                $where
    ...
    $changedgrades = $DB->get_records_sql("
            ...
            FROM (
                SELECT userid
                FROM {finalquiz_grades} qg
                WHERE quiz = :quizid
            UNION
                SELECT DISTINCT userid
                FROM {finalfinalquiz_attempts} quiza2
                WHERE
            ...
            LEFT JOIN {finalquiz_grades} qg ON qg.userid = users.userid AND qg.quiz = :quizid4
    ...
    foreach ($changedgrades as $changedgrade) {
        ...
        if (is_null($changedgrade->newgrade)) {
            ...
            $DB->insert_record('finalquiz_grades', $toinsert);
            ...
            $DB->update_record('finalquiz_grades', $toupdate);
        }
    }

    if (!empty($todelete)) {
        list($test, $params) = $DB->get_in_or_equal($todelete);
        $DB->delete_records_select('finalquiz_grades', 'quiz = ? AND userid ' . $test,
 ...
 *                      quizid   => (array|int) attempts in given finalquiz(s)
 *                      groupid  => (array|int) quizzes with some override for given group(s)
 *
 */
function finalquiz_update_open_attempts(array $conditions) {
    global $DB;
    ...
    if (isset($conditions['courseid'])) {
        ...
        $wheres[] = "quiza.quiz IN (SELECT q.id FROM {finalquiz} q WHERE q.course $incond)";
        list ($incond, $inparams) = $DB->get_in_or_equal($conditions['courseid'], SQL_PARAMS_NAMED, 'icid');
        $params = array_merge($params, $inparams);
        $iwheres[] = "iquiza.quiz IN (SELECT q.id FROM {finalquiz} q WHERE q.course $incond)";
    }
    ...
    if (isset($conditions['groupid'])) {
        ...
        $params = array_merge($params, $inparams);
        $wheres[] = "quiza.quiz IN (SELECT qo.quiz FROM {finalquiz_overrides} qo WHERE qo.groupid $incond)";
        list ($incond, $inparams) = $DB->get_in_or_equal($conditions['groupid'], SQL_PARAMS_NAMED, 'igid');
        $params = array_merge($params, $inparams);
        $iwheres[] = "iquiza.quiz IN (SELECT qo.quiz FROM {finalquiz_overrides} qo WHERE qo.groupid $incond)";
    }
    ...
    */
    ...
    if ($dbfamily == 'mysql') {
        $updatesql = "UPDATE {finalfinalquiz_attempts} quiza
                        JOIN {finalquiz} quiz ON quiz.id = quiza.quiz
        ...
        $updatesql = "UPDATE {finalfinalquiz_attempts} quiza
                        ...
                        FROM {finalquiz} quiz, ( $quizausersql ) quizauser
...
function quiz_get_attempt_usertime_sql($redundantwhereclauses = '') {
    ...
    $quizausersql = "
          SELECT iquiza.id,
           ...
           FROM {finalfinalquiz_attempts} iquiza
           JOIN {finalquiz} iquiz ON iquiz.id = iquiza.quiz
      LEFT JOIN {finalquiz_overrides} quo ON quo.quiz = iquiza.quiz AND quo.userid = iquiza.userid
      LEFT JOIN {groups_members} gm ON gm.userid = iquiza.userid
      LEFT JOIN {finalquiz_overrides} qgo1 ON qgo1.quiz = iquiza.quiz AND qgo1.groupid = gm.groupid AND qgo1.timeclose = 0
      LEFT JOIN {finalquiz_overrides} qgo2 ON qgo2.quiz = iquiza.quiz AND qgo2.groupid = gm.groupid AND qgo2.timeclose > 0
      LEFT JOIN {finalquiz_overrides} qgo3 ON qgo3.quiz = iquiza.quiz AND qgo3.groupid = gm.groupid AND qgo3.timelimit = 0
      LEFT JOIN {finalquiz_overrides} qgo4 ON qgo4.quiz = iquiza.quiz AND qgo4.groupid = gm.groupid AND qgo4.timelimit > 0
 ...
 */
function finalquiz_calculate_best_attempt($quiz, $attempts) {
    ...
    switch ($quiz->grademethod) {
        ...
        case FINALQUIZ_GRADEAVERAGE: // We need to do something with it.
        case FINALQUIZ_ATTEMPTLAST:
            foreach ($attempts as $attempt) {
                $final = $attempt;
            }
 ...
 */
function finalquiz_get_grading_options() {
    return array(
        FINALQUIZ_GRADEHIGHEST => get_string('gradehighest', 'finalquiz'),
        FINALQUIZ_GRADEAVERAGE => get_string('gradeaverage', 'finalquiz'),
        FINALQUIZ_ATTEMPTFIRST => get_string('attemptfirst', 'finalquiz'),
        FINALQUIZ_ATTEMPTLAST  => get_string('attemptlast', 'finalquiz')
    );
}

/**
 * @param int $option one of the values FINALQUIZ_GRADEHIGHEST, FINALQUIZ_GRADEAVERAGE,
 *      FINALQUIZ_ATTEMPTFIRST or FINALQUIZ_ATTEMPTLAST.
 * @return the lang string for that option.
 */
function finalquiz_get_grading_option_name($option) {
    $strings = finalquiz_get_grading_options();
 ...
 *      attempts.
 */
function finalquiz_get_overdue_handling_options() {
    return array(
        'autosubmit'  => get_string('overduehandlingautosubmit', 'finalquiz'),
        'graceperiod' => get_string('overduehandlinggraceperiod', 'finalquiz'),
        'autoabandon' => get_string('overduehandlingautoabandon', 'finalquiz'),
    );
}
 ...
 */
function finalquiz_get_user_image_options() {
    return array(
        FINALQUIZ_SHOWIMAGE_NONE  => get_string('shownoimage', 'finalquiz'),
        FINALQUIZ_SHOWIMAGE_SMALL => get_string('showsmallimage', 'finalquiz'),
        FINALQUIZ_SHOWIMAGE_LARGE => get_string('showlargeimage', 'finalquiz'),
    );
}
 ...
 */
function finalquiz_get_user_timeclose($courseid) {
    global $DB, $USER;
  ...
  FROM (
      SELECT quiz.id as quizid,
       ...
       FROM {finalquiz} quiz
  LEFT JOIN {finalquiz_overrides} quo on quiz.id = quo.quiz AND quo.userid = :userid
  LEFT JOIN {groups_members} gm ON gm.userid = :useringroupid
  LEFT JOIN {finalquiz_overrides} qgo on quiz.id = qgo.quiz AND qgo.groupid = gm.groupid
      WHERE quiz.course = :courseid
   GROUP BY quiz.id) v
       JOIN {finalquiz} q ON q.id = v.quizid";
 ...
 */
function finalquiz_questions_per_page_options() {
    $pageoptions = array();
    $pageoptions[0] = get_string('neverallononepage', 'finalquiz');
    $pageoptions[1] = get_string('everyquestion', 'finalquiz');
    for ($i = 2; $i <= FINALQUIZ_MAX_QPP_OPTION; ++$i) {
        $pageoptions[$i] = get_string('everynquestions', 'finalquiz', $i);
    }
    return $pageoptions;
}

/**
 * Get the human-readable name for a final quiz attempt state.
 * @param string $state one of the state constants like {@link finalquiz_attempt::IN_PROGRESS}.
 * @return string The lang string to describe that state.
 */
function finalquiz_attempt_state_name($state) {
    switch ($state) {
        case finalquiz_attempt::IN_PROGRESS:
            return get_string('stateinprogress', 'finalquiz');
        case finalquiz_attempt::OVERDUE:
            return get_string('stateoverdue', 'finalquiz');
        case finalquiz_attempt::FINISHED:
            return get_string('statefinished', 'finalquiz');
        case finalquiz_attempt::ABANDONED:
            return get_string('stateabandoned', 'finalquiz');
        default:
            throw new coding_exception('Unknown finalquiz attempt state.');
    }
}
...
/**
 * @param object $quiz the final quiz.
 * @param int $cmid the course_module object for this final quiz.
 ...
 */
function finalquiz_question_action_icons($quiz, $cmid, $question, $returnurl, $variant = null) {
    $html = finalquiz_question_preview_button($quiz, $question, false, $variant) . ' ' .
            finalquiz_question_edit_button($cmid, $question, $returnurl);
    return $html;
}

/**
 * @param int $cmid the course_module.id for this final quiz.
 * @param object $question the question.
 * @param string $returnurl url to return to after action is done.
 ...
 */
function finalquiz_question_edit_button($cmid, $question, $returnurl, $contentaftericon = '') {
    global $CFG, $OUTPUT;
 ...
 * @param object $question the question
 * @param int $variant which question variant to preview (optional).
 * @return moodle_url to preview this question with the options from this final quiz.
 */
function finalquiz_question_preview_url($quiz, $question, $variant = null) {
    // Get the appropriate display options.
    $displayoptions = mod_finalquiz_display_options::make_from_finalquiz($quiz,
            mod_finalquiz_display_options::DURING);
 ...
 */
function finalquiz_question_preview_button($quiz, $question, $label = false, $variant = null) {
    global $PAGE;
    if (!question_has_capability_on($question, 'use')) {
        return '';
    }

    return $PAGE->get_renderer('mod_finalquiz', 'edit')->question_preview_icon($quiz, $question, $label, $variant);
}
 ...
 */
function finalquiz_get_flag_option($attempt, $context) {
    global $USER;
...
/**
 * Work out what state this final quiz attempt is in - in the sense used by
 * finalquiz_get_review_options, not in the sense of $attempt->state.
 * @param object $quiz the final quiz settings
 * @param object $attempt the final quiz_attempt database row.
 * @return int one of the mod_finalquiz_display_options::DURING,
 *      IMMEDIATELY_AFTER, LATER_WHILE_OPEN or AFTER_CLOSE constants.
 */
function finalquiz_attempt_state($quiz, $attempt) {
    if ($attempt->state == finalquiz_attempt::IN_PROGRESS) {
        return mod_finalquiz_display_options::DURING;
    } else if ($quiz->timeclose && time() >= $quiz->timeclose) {
        return mod_finalquiz_display_options::AFTER_CLOSE;
    } else if (time() < $attempt->timefinish + 120) {
        return mod_finalquiz_display_options::IMMEDIATELY_AFTER;
    } else {
        return mod_finalquiz_display_options::LATER_WHILE_OPEN;
    }
}

/**
 * The the appropraite mod_finalquiz_display_options object for this attempt at this
 * final quiz right now.
 *
 * @param object $quiz the final quiz instance.
 * @param object $attempt the attempt in question.
 * @param $context the final quiz context.
 *
 * @return mod_finalquiz_display_options
 */
function finalquiz_get_review_options($quiz, $attempt, $context) {
    $options = mod_finalquiz_display_options::make_from_finalquiz($quiz, finalquiz_attempt_state($quiz, $attempt));

    $options->readonly = true;
    $options->flags = finalquiz_get_flag_option($attempt, $context);
    if (!empty($attempt->id)) {
        $options->questionreviewlink = new moodle_url('/mod/finalquiz/reviewquestion.php',
                array('attempt' => $attempt->id));
    }

    // Show a link to the comment box only for closed attempts.
    if (!empty($attempt->id) && $attempt->state == finalquiz_attempt::FINISHED && !$attempt->preview &&
            !is_null($context) && has_capability('mod/finalquiz:grade', $context)) {
        $options->manualcomment = question_display_options::VISIBLE;
        $options->manualcommentlink = new moodle_url('/mod/finalquiz/comment.php',
                array('attempt' => $attempt->id));
    }

    if (!is_null($context) && !$attempt->preview &&
            has_capability('mod/finalquiz:viewreports', $context) &&
...
/**
 * Combines the review options from a number of different final quiz attempts.
 * Returns an array of two ojects, so the suggested way of calling this
 * funciton is:
 * list($someoptions, $alloptions) = finalquiz_get_combined_reviewoptions(...)
 *
 * @param object $quiz the final quiz instance.
 ...
 *          for all attempts.
 */
function finalquiz_get_combined_reviewoptions($quiz, $attempts) {
    ...
    foreach ($attempts as $attempt) {
        $attemptoptions = mod_finalquiz_display_options::make_from_finalquiz($quiz,
                finalquiz_attempt_state($quiz, $attempt));
        foreach ($fields as $field) {
 ...
 */
function finalquiz_send_confirmation($recipient, $a) {
    ...
    $eventdata->component         = 'mod_finalquiz';
    $eventdata->name              = 'confirmation';
    $eventdata->notification      = 1;

    $eventdata->userfrom          = core_user::get_noreply_user();
    $eventdata->userto            = $recipient;
    $eventdata->subject           = get_string('emailconfirmsubject', 'finalquiz', $a);
    $eventdata->fullmessage       = get_string('emailconfirmbody', 'finalquiz', $a);
    $eventdata->fullmessageformat = FORMAT_PLAIN;
    $eventdata->fullmessagehtml   = '';

    $eventdata->smallmessage      = get_string('emailconfirmsmall', 'finalquiz', $a);
 ...
 * @return int|false as for {@link message_send()}.
 */
function finalquiz_send_notification($recipient, $submitter, $a) {
    ...
    $eventdata->userto            = $recipient;
    $eventdata->subject           = get_string('emailnotifysubject', 'finalquiz', $a);
    $eventdata->fullmessage       = get_string('emailnotifybody', 'finalquiz', $a);
    $eventdata->fullmessageformat = FORMAT_PLAIN;
    $eventdata->fullmessagehtml   = '';

    $eventdata->smallmessage      = get_string('emailnotifysmall', 'finalquiz', $a);
...
/**
 * Send all the requried messages when a final quiz attempt is submitted.
 *
 * @param object $course the course
 * @param object $quiz the final quiz
 * @param object $attempt this attempt just finished
 * @param object $context the final quiz context
 * @param object $cm the coursemodule for this final quiz
 *
 * @return bool true if all necessary messages were sent successfully, else false.
 */
function finalquiz_send_notification_messages($course, $quiz, $attempt, $context, $cm) {
    global $CFG, $DB;
    ...
    $sendconfirm = false;
    $notifyexcludeusers = '';
    if (has_capability('mod/finalquiz:emailconfirmsubmission', $context, $submitter, false)) {
    ...
    if (is_array($groups) && count($groups) > 0) {
        ...
        // If the user is not in a group, and the final quiz is set to group mode,
    ...
    $userstonotify = get_users_by_capability($context, 'mod/finalquiz:emailnotifysubmission',
    ...
    $a->quizreporturl   = $CFG->wwwroot . '/mod/finalquiz/report.php?id=' . $cm->id;
    $a->quizreportlink  = '<a href="' . $a->quizreporturl . '">' .
            format_string($quiz->name) . ' report</a>';
    $a->quizurl         = $CFG->wwwroot . '/mod/finalquiz/view.php?id=' . $cm->id;
    $a->quizlink        = '<a href="' . $a->quizurl . '">' . format_string($quiz->name) . '</a>';
    // Attempt info.
    $a->submissiontime  = userdate($attempt->timefinish);
    $a->timetaken       = format_time($attempt->timefinish - $attempt->timestart);
    $a->quizreviewurl   = $CFG->wwwroot . '/mod/finalquiz/review.php?attempt=' . $attempt->id;
    ...
    if (!empty($userstonotify)) {
        foreach ($userstonotify as $recipient) {
            $allok = $allok && finalquiz_send_notification($recipient, $submitter, $a);
        }
    }
    ...
    if ($sendconfirm) {
        $allok = $allok && finalquiz_send_confirmation($submitter, $a);
    }
...
/**
 * Send the notification message when a final quiz attempt becomes overdue.
 *
 * @param finalquiz_attempt $attemptobj all the data about the final quiz attempt.
 */
function finalquiz_send_overdue_message($attemptobj) {
    global $CFG, $DB;

    $submitter = $DB->get_record('user', array('id' => $attemptobj->get_userid()), '*', MUST_EXIST);

    if (!$attemptobj->has_capability('mod/finalquiz:emailwarnoverdue', $submitter->id, false)) {
    ...
    // the email message.
    $quizname = format_string($attemptobj->get_finalquiz_name());

    $deadlines = array();
    if ($attemptobj->get_finalquiz()->timelimit) {
        $deadlines[] = $attemptobj->get_attempt()->timestart + $attemptobj->get_finalquiz()->timelimit;
    }
    if ($attemptobj->get_finalquiz()->timeclose) {
        $deadlines[] = $attemptobj->get_finalquiz()->timeclose;
    }
    $duedate = min($deadlines);
    $graceend = $duedate + $attemptobj->get_finalquiz()->graceperiod;
    ...
    $eventdata->subject           = get_string('emailoverduesubject', 'finalquiz', $a);
    $eventdata->fullmessage       = get_string('emailoverduebody', 'finalquiz', $a);
    $eventdata->fullmessageformat = FORMAT_PLAIN;
    $eventdata->fullmessagehtml   = '';

    $eventdata->smallmessage      = get_string('emailoverduesmall', 'finalquiz', $a);
    $eventdata->contexturl        = $a->quizurl;
    $eventdata->contexturlname    = $a->quizname;
...
/**
 * Handle the finalquiz_attempt_submitted event.
 *
 * This sends the confirmation and notification messages, if required.
 *
 * @param object $event the event object.
 */
function finalquiz_attempt_submitted_handler($event) {
    global $DB;

    $course  = $DB->get_record('course', array('id' => $event->courseid));
    $attempt = $event->get_record_snapshot('finalfinalquiz_attempts', $event->objectid);
    $quiz    = $event->get_record_snapshot('finalquiz', $attempt->quiz);
    $cm      = get_coursemodule_from_id('finalquiz', $event->get_context()->instanceid, $event->courseid);
    ...
    return finalquiz_send_notification_messages($course, $quiz, $attempt,
 ...
 *
 * @param object $event the event object.
 * @deprecated since 2.6, see {@link \mod_finalquiz\group_observers::group_member_added()}.
 */
function finalquiz_groups_member_added_handler($event) {
    debugging('finalquiz_groups_member_added_handler() is deprecated, please use ' .
        '\mod_finalquiz\group_observers::group_member_added() instead.', DEBUG_DEVELOPER);
    finalquiz_update_open_attempts(array('userid'=>$event->userid, 'groupid'=>$event->groupid));
}
 ...
 *
 * @param object $event the event object.
 * @deprecated since 2.6, see {@link \mod_finalquiz\group_observers::group_member_removed()}.
 */
function finalquiz_groups_member_removed_handler($event) {
    debugging('finalquiz_groups_member_removed_handler() is deprecated, please use ' .
        '\mod_finalquiz\group_observers::group_member_removed() instead.', DEBUG_DEVELOPER);
 ...
 *
 * @param object $event the event object.
 * @deprecated since 2.6, see {@link \mod_finalquiz\group_observers::group_deleted()}.
 */
function finalquiz_groups_group_deleted_handler($event) {
    global $DB;
    debugging('finalquiz_groups_group_deleted_handler() is deprecated, please use ' .
        '\mod_finalquiz\group_observers::group_deleted() instead.', DEBUG_DEVELOPER);
    finalquiz_process_group_deleted_in_course($event->courseid);
}
 ...
 * @return void
 */
function finalquiz_process_group_deleted_in_course($courseid) {
    global $DB;
    ...
    $sql = "SELECT o.id, o.quiz
              FROM {finalquiz_overrides} o
              JOIN {finalquiz} quiz ON quiz.id = o.quiz
    ...
    $DB->delete_records_list('finalquiz_overrides', 'id', array_keys($records));
    finalquiz_update_open_attempts(array('quizid' => array_unique(array_values($records))));
}
 ...
 * @deprecated since 2.6, see {@link \mod_finalquiz\group_observers::group_member_removed()}.
 */
function finalquiz_groups_members_removed_handler($event) {
    debugging('finalquiz_groups_members_removed_handler() is deprecated, please use ' .
        '\mod_finalquiz\group_observers::group_member_removed() instead.', DEBUG_DEVELOPER);
    if ($event->userid == 0) {
        finalquiz_update_open_attempts(array('courseid'=>$event->courseid));
    } else {
        finalquiz_update_open_attempts(array('courseid'=>$event->courseid, 'userid'=>$event->userid));
    }
}

/**
 * Get the information about the standard final quiz JavaScript module.
 * @return array a standard jsmodule structure.
 */
function finalquiz_get_js_module() {
    global $PAGE;

    return array(
        'name' => 'mod_finalquiz',
        'fullpath' => '/mod/finalquiz/module.js',
        'requires' => array('base', 'dom', 'event-delegate', 'event-key',
                'core_question_engine', 'moodle-core-formchangechecker'),
        'strings' => array(
            array('cancel', 'moodle'),
            array('flagged', 'question'),
            array('functiondisabledbysecuremode', 'finalquiz'),
            array('startattempt', 'finalquiz'),
            array('timesup', 'finalquiz'),
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_display_options extends question_display_options {
    /**#@+
    ...
    /**
     * Set up the various options from the final quiz settings, and a time constant.
     * @param object $quiz the final quiz settings.
     * @param int $one of the {@link DURING}, {@link IMMEDIATELY_AFTER},
     * {@link LATER_WHILE_OPEN} or {@link AFTER_CLOSE} constants.
     * @return mod_finalquiz_display_options set up appropriately.
     */
    public static function make_from_finalquiz($quiz, $when) {
        $options = new self();
 ...
 * a particular final quiz.
 *
 * @copyright  2010 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qubaids_for_finalquiz extends qubaid_join {
    public function __construct($quizid, $includepreviews = true, $onlyfinished = false) {
        ...
        if ($onlyfinished) {
            ...
            $params['statefinished'] = finalquiz_attempt::FINISHED;
        }

        parent::__construct('{finalfinalquiz_attempts} quiza', 'quiza.uniqueid', $where, $params);
    }
}

/**
 * A {@link qubaid_condition} for finding all the question usages belonging to a particular user and final quiz combination.
 *
 * @copyright  2018 Andrew Nicols <andrwe@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qubaids_for_finalquiz_user extends qubaid_join {
    /**
     * Constructor for this qubaid.
     *
     * @param   int     $quizid The finalquiz to search.
 ...
 * @return string
 */
function finalquiz_question_tostring($question, $showicon = false, $showquestiontext = true) {
    $result = '';
 ...
 */
function finalquiz_require_question_use($questionid) {
    global $DB;
 ...
 * Verify that the question exists, and the user has permission to use it.
 * @param object $quiz the final quiz settings.
 * @param int $slot which question in the final quiz to test.
 * @return bool whether the user can use this question.
 */
function finalquiz_has_question_use($quiz, $slot) {
    global $DB;
    $question = $DB->get_record_sql("
            SELECT q.*
              FROM {finalquiz_slots} slot
...
/**
 * Add a question to a final quiz
 *
 * Adds a question to a final quiz by updating $quiz as well as the
 * finalquiz and finalquiz_slots tables. It also adds a page break if required.
 * @param int $questionid The id of the question to be added
 * @param object $quiz The extended final quiz object as used by edit.php
 *      This is updated by this function
 * @param int $page Which page in final quiz to add the question on. If 0 (default),
 *      add at the end
 * @param float $maxmark The maximum mark to set for this question. (Optional,
 *      defaults to question.defaultmark.
 * @return bool false if the question was already in the final quiz
 */
function finalquiz_add_finalquiz_question($questionid, $quiz, $page = 0, $maxmark = null) {
    global $DB;
    ...
    if ($questiontype == 'random') {
        throw new coding_exception(
                'Adding "random" questions via finalquiz_add_finalquiz_question() is deprecated. Please use finalquiz_add_random_questions().'
        );
    }

    $slots = $DB->get_records('finalquiz_slots', array('quizid' => $quiz->id),
    ...
    if (is_int($page) && $page >= 1) {
        ...
        finalquiz_update_section_firstslots($quiz->id, 1, max($lastslotbefore, 1));
    ...
    $DB->insert_record('finalquiz_slots', $slot);
    $trans->allow_commit();
}
 ...
 *
 * @param int $quizid the id of a final quiz
 ...
 */
function finalquiz_update_section_firstslots($quizid, $direction, $afterslot, $beforeslot = null) {
    global $DB;
    ...
    $firstslotschanges = $DB->get_records_select_menu('finalquiz_sections',
            $where, $params, '', 'firstslot, firstslot + ?');
    update_field_with_unique_index('finalquiz_sections', 'firstslot', $firstslotschanges, ['quizid' => $quizid]);
}

/**
 * Add a random question to the final quiz at a given point.
 * @param stdClass $quiz the final quiz settings.
 ...
 */
function finalquiz_add_random_questions($quiz, $addonpage, $categoryid, $number,
    ...
    for ($i = 0; $i < $number; $i++) {
        ...
        if (!$question = array_shift($existingquestions)) {
            ...
            if (!isset($question->id)) {
                print_error('cannotinsertrandomquestion', 'finalquiz');
            }
        }
        ...
        $randomslot = new \mod_finalquiz\local\structure\slot_random($randomslotdata);
 ...
 *
 * @param  stdClass $quiz       finalquiz object
 * @param  stdClass $course     course object
 * @param  stdClass $cm         course module object
 * @param  stdClass $context    context object
 * @since Moodle 3.1
 */
function finalquiz_view($quiz, $course, $cm, $context) {
    ...
    $event = \mod_finalquiz\event\course_module_viewed::create($params);
    $event->add_record_snapshot('finalquiz', $quiz);
    $event->trigger();
 ...
 *
 * @param  finalquiz $quizobj quiz object
 * @param  finalquiz_access_manager $accessmanager quiz access manager
 ...
 * @throws moodle_finalquiz_exception
 * @since Moodle 3.1
 */
function finalquiz_validate_new_attempt(finalquiz $quizobj, finalquiz_access_manager $accessmanager, $forcenew, $page, $redirect) {
    global $DB, $USER;
    $timenow = time();
    ...
    if (!$quizobj->is_preview_user()) {
        $quizobj->require_capability('mod/finalquiz:attempt');
    }
    ...
    if ($lastattempt && ($lastattempt->state == finalquiz_attempt::IN_PROGRESS ||
            $lastattempt->state == finalquiz_attempt::OVERDUE)) {
        ...
        if ($lastattempt->state == finalquiz_attempt::ABANDONED || $lastattempt->state == finalquiz_attempt::FINISHED) {
            if ($redirect) {
                redirect($quizobj->review_url($lastattempt->id));
            } else {
                throw new moodle_finalquiz_exception($quizobj, 'attemptalreadyclosed');
            }
        }
 ...
 * @since  Moodle 3.1
 */
function finalquiz_prepare_and_start_new_attempt(finalquiz $quizobj, $attemptnumber, $lastattempt, $offlineattempt = false) {
    global $DB, $USER;
    ...
    $attempt = finalquiz_create_attempt($quizobj, $attemptnumber, $lastattempt, $timenow, $quizobj->is_preview_user());

    if (!($quizobj->get_finalquiz()->attemptonlast && $lastattempt)) {
        $attempt = finalquiz_start_new_attempt($quizobj, $quba, $attempt, $attemptnumber, $timenow);
    } else {
        $attempt = finalquiz_start_attempt_built_on_last($quba, $attempt, $lastattempt);
    }
    ...
    $attempt = finalquiz_attempt_save_started($quizobj, $quba, $attempt);
 ...
 * event for final quiz.
 *
 * @param calendar_event $event The calendar event to check
 * @return bool
 */
function finalquiz_is_overriden_calendar_event(\calendar_event $event) {
    global $DB;
    ...
    return $DB->record_exists('finalquiz_overrides', $overrideparams);
}

/**
 * Retrieves tag information for the given list of final quiz slot ids.
 ...
 * ]
 *
 * @param int[] $slotids The list of id for the final quiz slots.
 * @return array[] List of finalquiz_slot_tags records indexed by slot id.
 */
function finalquiz_retrieve_tags_for_slot_ids($slotids) {
    global $DB;
    ...
    $slottags = $DB->get_records_list('finalquiz_slot_tags', 'slotid', $slotids);
...
/**
 * Retrieves tag information for the given final quiz slot.
 * A final quiz slot have some tags if and only if it is representing a random question by tags.
 *
 * @param int $slotid The id of the final quiz slot.
 * @return stdClass[] List of finalquiz_slot_tags records.
 */
function finalquiz_retrieve_slot_tags($slotid) {
    $slottags = finalquiz_retrieve_tags_for_slot_ids([$slotid]);
    return $slottags[$slotid];
}

/**
 * Retrieves tag ids for the given final quiz slot.
 * A final quiz slot have some tags if and only if it is representing a random question by tags.
 *
 * @param int $slotid The id of the final quiz slot.
 * @return int[]
 */
function finalquiz_retrieve_slot_tag_ids($slotid) {
    $tags = finalquiz_retrieve_slot_tags($slotid);
...
/**
 * Get final quiz attempt and handling error.
 *
 * @param int $attemptid the id of the current attempt.
 * @param int|null $cmid the course_module id for this final quiz.
 * @return finalquiz_attempt $attemptobj all the data about the final quiz attempt.
 * @throws moodle_exception
 */
function finalquiz_create_attempt_handling_errors($attemptid, $cmid = null) {
    try {
        $attempobj = finalquiz_attempt::create($attemptid);
    } catch (moodle_exception $e) {
        if (!empty($cmid)) {
            list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'finalquiz');
            $continuelink = new moodle_url('/mod/finalquiz/view.php', array('id' => $cmid));
            $context = context_module::instance($cm->id);
            if (has_capability('mod/finalquiz:preview', $context)) {
                throw new moodle_exception('attempterrorcontentchange', 'finalquiz', $continuelink);
            } else {
                throw new moodle_exception('attempterrorcontentchangeforuser', 'finalquiz', $continuelink);
            }
        } else {
            throw new moodle_exception('attempterrorinvalid', 'finalquiz');
        }
    }
