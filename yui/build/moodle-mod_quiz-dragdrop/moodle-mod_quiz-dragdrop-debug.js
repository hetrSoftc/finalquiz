YUI.add('moodle-mod_finalquiz-dragdrop', function (Y, NAME) {
 ...
 *
 * @class M.mod_finalquiz.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'moodle-mod_finalquiz-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_finalquiz-dragdrop');
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_finalquiz.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {
                        // Ignore.
                    }
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                M.mod_finalquiz.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_finalquiz-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_finalquiz.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Moodle.mod_finalquiz.util.slot.getId(dragnode));
        ...
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_finalquiz.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_finalquiz.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_finalquiz.quizbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_finalquiz.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_finalquiz-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_finalquiz = M.mod_finalquiz || {};
M.mod_finalquiz.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_finalquiz-util-base",
        "moodle-mod_finalquiz-util-page",
