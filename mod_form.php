 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_mod_form extends moodleform_mod {
    /** @var array options to be used with date_time_selector fields in the final quiz. */
    public static $datefieldoptions = array('optional' => true);

    protected $_feedbacks;
    protected static $reviewfields = array(); // Initialised in the constructor.

    /** @var int the max number of attempts allowed in any user or group override on this final quiz. */
    protected $maxattemptsanyoverride = null;

    public function __construct($current, $section, $cm, $course) {
        self::$reviewfields = array(
            'attempt'          => array('theattempt', 'finalquiz'),
            'correctness'      => array('whethercorrect', 'question'),
            'marks'            => array('marks', 'finalquiz'),
            ...
            'overallfeedback'  => array('reviewoverallfeedback', 'finalquiz'),
        );
    ...
    protected function definition() {
        ...
        $quizconfig = get_config('finalquiz');
        $mform = $this->_form;
        ...
        // Introduction.
        $this->standard_intro_elements(get_string('introduction', 'finalquiz'));

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'timing', get_string('timing', 'finalquiz'));

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen', get_string('quizopen', 'finalquiz'),
                self::$datefieldoptions);
        $mform->addHelpButton('timeopen', 'quizopenclose', 'finalquiz');

        $mform->addElement('date_time_selector', 'timeclose', get_string('quizclose', 'finalquiz'),
                self::$datefieldoptions);

        // Time limit.
        $mform->addElement('duration', 'timelimit', get_string('timelimit', 'finalquiz'),
                array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'finalquiz');
        $mform->setAdvanced('timelimit', $quizconfig->timelimit_adv);
        $mform->setDefault('timelimit', $quizconfig->timelimit);

        // What to do with overdue attempts.
        $mform->addElement('select', 'overduehandling', get_string('overduehandling', 'finalquiz'),
                finalquiz_get_overdue_handling_options());
        $mform->addHelpButton('overduehandling', 'overduehandling', 'finalquiz');
        ...
        // Grace period time.
        $mform->addElement('duration', 'graceperiod', get_string('graceperiod', 'finalquiz'),
                array('optional' => true));
        $mform->addHelpButton('graceperiod', 'graceperiod', 'finalquiz');
        ...
        $mform->addElement('select', 'attempts', get_string('attemptsallowed', 'finalquiz'),
                $attemptoptions);
        $mform->setAdvanced('attempts', $quizconfig->attempts_adv);
        $mform->setDefault('attempts', $quizconfig->attempts);

        // Grading method.
        $mform->addElement('select', 'grademethod', get_string('grademethod', 'finalquiz'),
                finalquiz_get_grading_options());
        $mform->addHelpButton('grademethod', 'grademethod', 'finalquiz');
        ...
        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'layouthdr', get_string('layout', 'finalquiz'));

        $pagegroup = array();
        $pagegroup[] = $mform->createElement('select', 'questionsperpage',
                get_string('newpage', 'finalquiz'), quiz_questions_per_page_options(), array('id' => 'id_questionsperpage'));
        $mform->setDefault('questionsperpage', $quizconfig->questionsperpage);

        if (!empty($this->_cm)) {
            $pagegroup[] = $mform->createElement('checkbox', 'repaginatenow', '',
                    get_string('repaginatenow', 'finalquiz'), array('id' => 'id_repaginatenow'));
        }

        $mform->addGroup($pagegroup, 'questionsperpagegrp',
                get_string('newpage', 'finalquiz'), null, false);
        $mform->addHelpButton('questionsperpagegrp', 'newpage', 'finalquiz');
        $mform->setAdvanced('questionsperpagegrp', $quizconfig->questionsperpage_adv);

        // Navigation method.
        $mform->addElement('select', 'navmethod', get_string('navmethod', 'finalquiz'),
                finalquiz_get_navigation_options());
        $mform->addHelpButton('navmethod', 'navmethod', 'finalquiz');
        $mform->setAdvanced('navmethod', $quizconfig->navmethod_adv);
        $mform->setDefault('navmethod', $quizconfig->navmethod);

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'interactionhdr', get_string('questionbehaviour', 'finalquiz'));

        // Shuffle within questions.
        $mform->addElement('selectyesno', 'shuffleanswers', get_string('shufflewithin', 'finalquiz'));
        $mform->addHelpButton('shuffleanswers', 'shufflewithin', 'finalquiz');
        ...
        $redochoices = array(0 => get_string('no'), 1 => get_string('canredoquestionsyes', 'finalquiz'));
        $mform->addElement('select', 'canredoquestions', get_string('canredoquestions', 'finalquiz'), $redochoices);
        $mform->addHelpButton('canredoquestions', 'canredoquestions', 'finalquiz');
        ...
        $mform->addHelpButton('attemptonlast', 'eachattemptbuildsonthelast', 'finalquiz');
        ...
        $mform->addHelpButton('reviewoptionshdr', 'reviewoptionsheading', 'finalquiz');

        // Review options.
        $this->add_review_options_group($mform, $quizconfig, 'during',
                mod_finalquiz_display_options::DURING, true);
        $this->add_review_options_group($mform, $quizconfig, 'immediately',
                mod_finalquiz_display_options::IMMEDIATELY_AFTER);
        ...
        // Show user picture.
        $mform->addElement('select', 'showuserpicture', get_string('showuserpicture', 'finalquiz'),
                finalquiz_get_user_image_options());
        $mform->addHelpButton('showuserpicture', 'showuserpicture', 'finalquiz');
        ...
        $mform->addElement('select', 'decimalpoints', get_string('decimalplaces', 'finalquiz'),
                $options);
        $mform->addHelpButton('decimalpoints', 'decimalplaces', 'finalquiz');
        ...
        $mform->addElement('select', 'questiondecimalpoints',
                get_string('decimalplacesquestion', 'finalquiz'), $options);
        $mform->addHelpButton('questiondecimalpoints', 'decimalplacesquestion', 'finalquiz');
        ...
        // Show blocks during final quiz attempt.
        $mform->addElement('selectyesno', 'showblocks', get_string('showblocks', 'finalquiz'));
        $mform->addHelpButton('showblocks', 'showblocks', 'finalquiz');
        $mform->setAdvanced('showblocks', $quizconfig->showblocks_adv);
        $mform->setDefault('showblocks', $quizconfig->showblocks);

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'security', get_string('extraattemptrestrictions', 'finalquiz'));

        // Require password to begin final quiz attempt.
        $mform->addElement('passwordunmask', 'quizpassword', get_string('requirepassword', 'finalquiz'));
        $mform->setType('quizpassword', PARAM_TEXT);
        $mform->addHelpButton('quizpassword', 'requirepassword', 'finalquiz');
        $mform->setAdvanced('quizpassword', $quizconfig->password_adv);
        $mform->setDefault('quizpassword', $quizconfig->password);

        // IP address.
        $mform->addElement('text', 'subnet', get_string('requiresubnet', 'finalquiz'));
        $mform->setType('subnet', PARAM_TEXT);
        $mform->addHelpButton('subnet', 'requiresubnet', 'finalquiz');
        $mform->setAdvanced('subnet', $quizconfig->subnet_adv);
        $mform->setDefault('subnet', $quizconfig->subnet);

        // Enforced time delay between final quiz attempts.
        $mform->addElement('duration', 'delay1', get_string('delay1st2nd', 'finalquiz'),
                array('optional' => true));
        $mform->addHelpButton('delay1', 'delay1st2nd', 'finalquiz');
        ...
        $mform->addElement('duration', 'delay2', get_string('delaylater', 'finalquiz'),
                array('optional' => true));
        $mform->addHelpButton('delay2', 'delaylater', 'finalquiz');
        ...
        $mform->addElement('select', 'browsersecurity', get_string('browsersecurity', 'finalquiz'),
                finalquiz_access_manager::get_browser_security_choices());
        $mform->addHelpButton('browsersecurity', 'browsersecurity', 'finalquiz');
        ...
        finalquiz_access_manager::add_settings_form_fields($this, $mform);

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'overallfeedbackhdr', get_string('overallfeedback', 'finalquiz'));
        $mform->addHelpButton('overallfeedbackhdr', 'overallfeedback', 'finalquiz');
        ...
        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1,
                ...
                get_string('addmoreoverallfeedbacks', 'finalquiz'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore($mform->createElement('editor',
                "feedbacktext[$nextel]", get_string('feedback', 'finalquiz'), array('rows' => 3),
                array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true,
                      'context' => $this->context)),
                'boundary_add_fields');
        $mform->insertElementBefore($mform->createElement('static',
                'gradeboundarystatic2', get_string('gradeboundary', 'finalquiz'), '0%'),
                'boundary_add_fields');
        ...
        // final quiz has its own setting, so use that.
        $mform->setDefault('grade', $quizconfig->maximumgrade);

        // -------------------------------------------------------------------------------
        $this->add_action_buttons();

        $PAGE->requires->yui_module('moodle-mod_finalquiz-modform', 'M.mod_finalquiz.modform.init');
    }
    ...
    protected function add_review_options_group($mform, $quizconfig, $whenname,
        ...
        $mform->addGroup($group, $whenname . 'optionsgrp',
                get_string('review' . $whenname, 'finalquiz'), null, false);
    ...
    public function data_preprocessing(&$toform) {
        ...
        if (!empty($toform['instance'])) {
            $accesssettings = finalquiz_access_manager::load_settings($toform['instance']);
    ...
    public function validation($data, $files) {
        ...
        if ($data['timeopen'] != 0 && $data['timeclose'] != 0 &&
            ...
            $errors['timeclose'] = get_string('closebeforeopen', 'finalquiz');
        }
        ...
        if ($data['overduehandling'] == 'graceperiod') {
            ...
            if ($data['graceperiod'] <= $graceperiodmin) {
                $errors['graceperiod'] = get_string('graceperiodtoosmall', 'finalquiz', format_time($graceperiodmin));
            }
        }
        ...
        if (array_key_exists('completion', $data) && $data['completion'] == COMPLETION_TRACKING_AUTOMATIC) {
            ...
            if ($completionpass && (empty($data['gradepass']) || grade_floatval($data['gradepass']) == 0)) {
                if (isset($data['completionpass'])) {
                    $errors['completionpassgroup'] = get_string('gradetopassnotset', 'finalquiz');
                } else {
                    $errors['gradepass'] = get_string('gradetopassmustbeset', 'finalquiz');
                }
            }
        }
        ...
        while (!empty($data['feedbackboundaries'][$i] )) {
                ...
                if ($boundary[strlen($boundary) - 1] == '%') {
                    ...
                    if (is_numeric($boundary)) {
                        ...
                        $errors["feedbackboundaries[$i]"] =
                                get_string('feedbackerrorboundaryformat', 'finalquiz', $i + 1);
                    }
            ...
            if (is_numeric($boundary) && $boundary <= 0 || $boundary >= $data['grade'] ) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrorboundaryoutofrange', 'finalquiz', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrororder', 'finalquiz', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] =
                            get_string('feedbackerrorjunkinboundary', 'finalquiz', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] =
                        get_string('feedbackerrorjunkinfeedback', 'finalquiz', $i + 1);
            }
        }
        ...
        // Any other rule plugins.
        $errors = finalquiz_access_manager::validate_settings_form_fields($errors, $data, $files, $this);
    ...
    public function add_completion_rules() {
        ...
        $group = array();
        $group[] = $mform->createElement('advcheckbox', 'completionpass', null, get_string('completionpass', 'finalquiz'),
                array('group' => 'cpass'));
        $mform->disabledIf('completionpass', 'completionusegrade', 'notchecked');
        $group[] = $mform->createElement('advcheckbox', 'completionattemptsexhausted', null,
                get_string('completionattemptsexhausted', 'finalquiz'),
                array('group' => 'cattempts'));
        $mform->disabledIf('completionattemptsexhausted', 'completionpass', 'notchecked');
        $mform->addGroup($group, 'completionpassgroup', get_string('completionpass', 'finalquiz'), ' &nbsp; ', false);
        $mform->addHelpButton('completionpassgroup', 'completionpass', 'finalquiz');
    ...
    public function get_max_attempts_for_any_override() {
        ...
        if (empty($this->_instance)) {
            // final Quiz not created yet, so no overrides.
            return 1;
        }
