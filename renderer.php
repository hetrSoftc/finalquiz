 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param finalquiz_attempt $attemptobj an instance of finalquiz_attempt.
     ...
     * @param mod_finalquiz_display_options $displayoptions instance of mod_finalquiz_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(finalquiz_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_finalquiz_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(finalquiz_attempt $attemptobj, $slot, $seq,
            mod_finalquiz_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(finalquiz_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_finalquiz_name(), true,
                                  array("context" => $attemptobj->get_finalquizobj()->get_context())));
     ...
     *
     * @param finalquiz_attempt $attemptobj instance of finalquiz_attempt
     ...
     * @param mod_finalquiz_display_options $displayoptions instance of mod_finalquiz_display_options
     */
    public function questions(finalquiz_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_finalquiz_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_finalquiz.init_review_form', null, false,
                finalquiz_get_js_module());
     ...
     * @param finalquiz_attempt $attemptobj instance of finalquiz_attempt
     */
    public function finish_review_link(finalquiz_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_finalquiz.secure_window.init_close_button',
                    array($url), false, finalquiz_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'finalquiz'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_finalquiz-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'finalquiz'),
                    array('class' => 'mod_finalquiz-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(finalquiz_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'finalquiz'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_finalquiz-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'finalquiz'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_finalquiz-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the finalquiz timer.
     * @return string HTML content.
     */
    public function countdown_timer(finalquiz_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the finalquiz to be submitted immediately.
                $timerstartvalue = max($timerstartvalue, 1);
            }
            $this->initialise_timer($timerstartvalue, $ispreview);
        }

        return html_writer::tag('div', get_string('timeleft', 'finalquiz') . ' ' .
                html_writer::tag('span', '', array('id' => 'finalquiz-time-left')),
                array('id' => 'finalquiz-timer', 'role' => 'timer',
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'finalquiz'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param finalquiz_nav_panel_base $panel instance of finalquiz_nav_panel_base
     */
    public function navigation_panel(finalquiz_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_finalquiz.nav.init', null, false,
                finalquiz_get_js_module());
    ...
    /**
     * Display a final quiz navigation button.
     *
     * @param finalquiz_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_finalquiz_nav_question_button(finalquiz_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn', 'btn-secondary');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'finalquiz');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'finalquiz', $a);
     ...
     *
     * @param finalquiz_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_finalquiz_nav_section_heading(finalquiz_nav_section_heading $heading) {
        return $this->heading($heading->heading, 3, 'mod_finalquiz-section-heading');
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_finalquiz_links_to_other_attempts $links
     */
    protected function render_mod_finalquiz_links_to_other_attempts(
            mod_finalquiz_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(finalquiz $quizobj, mod_finalquiz_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($quizobj->get_finalquiz_name(), true,
                                  array("context" => $quizobj->get_context())));
        $output .= $this->finalquiz_intro($quizobj->get_finalquiz(), $quizobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param finalquiz_attempt $attemptobj Instance of finalquiz_attempt
     * @param int $page Current page number
     * @param finalquiz_access_manager $accessmanager Instance of finalquiz_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->finalquiz_notices($messages);
     ...
     */
    public function finalquiz_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'finalquiz'), 3) .
                $this->access_messages($messages), 'finalquizaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param finalquiz_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_finalquiz()->navmethod;
    ...
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'finalquiz'), 'class' => 'mod_finalquiz-prev-nav btn btn-secondary'));
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'finalquiz');
        } else {
            $nextlabel = get_string('navigatenext', 'finalquiz');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_finalquiz-next-nav btn btn-primary'));
        $output .= html_writer::end_tag('div');
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'finalquiz'), 'class' => 'mod_finalquiz-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_finalquiz.timer.init', $options, false, finalquiz_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'finalquiz'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'finalquiz'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_finalquiz.secure_window.close',
                array($url, $delay), false, finalquiz_get_js_module());
     ...
     *
     * @param finalquiz_attempt $attemptobj
     * @param mod_finalquiz_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_finalquiz_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'finalquiz'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->head = array(get_string('question', 'finalquiz'), get_string('status', 'finalquiz'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'finalquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == finalquiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'finalquiz'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'finalquiz'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == finalquiz_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'finalquiz'), null,
                    get_string('submitallandfinish', 'finalquiz')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == finalquiz_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'finalquiz', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'finalquiz', userdate($duedate));
        }
     ...
     * @param mod_finalquiz_view_object $viewobj
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        ...
        $output .= $this->box($this->view_page_buttons($viewobj), 'finalquizattempt');
        return $output;
    }
     ...
     * @param mod_finalquiz_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_finalquiz_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'finalquiz'), 'get',
     ...
     * @param mod_finalquiz_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_finalquiz_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_finalquiz_preflight_check_form from ' .
                    'quiz_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' finalquizstartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' finalquizsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_finalquiz/preflightcheck', 'init',
                array('.finalquizstartbuttondiv [type=submit]', get_string('startattempt', 'finalquiz'),
                       '#mod_finalquiz_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'finalquiz'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editfinalquiz', 'finalquiz'), 'get');
        }
    ...
    public function view_page_guest($course, $quiz, $cm, $context, $messages) {
        ...
        $guestno = html_writer::tag('p', get_string('guestsno', 'finalquiz'));
    ...
    public function view_page_notenrolled($course, $quiz, $cm, $context, $messages) {
        ...
        $output = '';
        $output .= $this->view_information($quiz, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'finalquiz'));
    ...
    public function view_information($quiz, $cm, $context, $messages) {
        ...
        $output .= $this->finalquiz_intro($quiz, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'finalquizinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/finalquiz:viewreports', $context)) {
            if ($strattemptnum = $this->finalquiz_attempt_summary_link_to_reports($quiz, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'finalquizattemptcounts'));
            }
        }
        return $output;
    }
     ...
     * @return string HTML to output.
     */
    public function finalquiz_intro($quiz, $cm) {
        if (html_is_blank($quiz->intro)) {
            return '';
        }

        return $this->box(format_module_intro('finalquiz', $quiz, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'finalquiz'), 3);
    }
    ...
    public function view_table($quiz, $context, $viewobj) {
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable finalquizattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'finalquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'finalquiz');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'finalquiz') . ' / ' .
                    finalquiz_format_grade($quiz, $quiz->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    finalquiz_format_grade($quiz, $quiz->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'finalquiz');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'finalquiz');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'finalquiz');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = finalquiz_format_grade($quiz, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = finalquiz_rescale_grade($attemptobj->get_sum_marks(), $quiz, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == finalquiz_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $quiz->grademethod == FINALQUIZ_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = finalquiz_format_grade($quiz, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = finalquiz_feedback_for_grade($attemptgrade, $quiz, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param finalquiz_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case finalquiz_attempt::OVERDUE:
                return get_string('stateoverdue', 'finalquiz') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'finalquiz',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case finalquiz_attempt::FINISHED:
                return get_string('statefinished', 'finalquiz') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'finalquiz',
    ...
    /**
     * Generates data pertaining to finalquiz results
     *
    ...
    public function view_result_info($quiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = finalquiz_get_grading_option_name($quiz->grademethod);
                $a->mygrade = finalquiz_format_grade($quiz, $viewobj->mygrade);
                $a->quizgrade = finalquiz_format_grade($quiz, $quiz->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'finalquiz', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = finalquiz_format_grade($quiz, $viewobj->mygrade);
                $a->maxgrade = finalquiz_format_grade($quiz, $quiz->grade);
                $a = get_string('outofshort', 'finalquiz', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'finalquiz', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'finalquiz'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'finalquizteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'finalquiz'), 3);
            $resultinfo .= html_writer::div(
                    finalquiz_feedback_for_grade($viewobj->mygrade, $quiz, $context),
                    'finalquizgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'finalquiz'));
            $button->add_action(new popup_action('click', $url, 'finalquizpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'finalquiz'),
                    array('title' => get_string('reviewthisattempt', 'finalquiz')));
        }
    }
    ...
    /**
     * Returns the same as {@link finalquiz_num_attempt_summary()} but wrapped in a link
     * to the quiz reports.
     *
     ...
     */
    public function finalquiz_attempt_summary_link_to_reports($quiz, $cm, $context,
        ...
        $url = new moodle_url('/mod/finalquiz/report.php', array(
                'id' => $cm->id, 'mode' => finalquiz_report_default_report($context)));
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'finalquiz'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'finalquiz'), FORMAT_MARKDOWN, $options);
...
class mod_finalquiz_links_to_other_attempts implements renderable {
    /**
...
class mod_finalquiz_view_object {
    ...
    public $attemptobjs;
    /** @var finalquiz_access_manager $accessmanager contains various access rules. */
