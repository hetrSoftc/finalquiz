...
M.mod_finalquiz.init_attempt_form = function(Y) {
    M.core_question_engine.init_form(Y, '#responseform');
    Y.on('submit', M.mod_finalquiz.timer.stop, '#responseform');
    M.core_formchangechecker.init({formid: 'responseform'});
};

M.mod_finalquiz.init_review_form = function(Y) {
    M.core_question_engine.init_form(Y, '.questionflagsaveform');
    Y.on('submit', function(e) { e.halt(); }, '.questionflagsaveform');
};

M.mod_finalquiz.init_comment_popup = function(Y) {
...
M.mod_finalquiz.timer = {
    ...
    init: function(Y, start, preview) {
        M.mod_finalquiz.timer.Y = Y;
        M.mod_finalquiz.timer.endtime = M.pageloadstarttime.getTime() + start*1000;
        M.mod_finalquiz.timer.preview = preview;
        M.mod_finalquiz.timer.update();
     ...
     */
    stop: function(e) {
        if (M.mod_finalquiz.timer.timeoutid) {
            clearTimeout(M.mod_finalquiz.timer.timeoutid);
        }
    },
    ...
    update: function() {
        var Y = M.mod_finalquiz.timer.Y;
        var secondsleft = Math.floor((M.mod_finalquiz.timer.endtime - new Date().getTime())/1000);

        // If time has expired, set the hidden form field that says time has expired and submit
        if (secondsleft < 0) {
            M.mod_finalquiz.timer.stop(null);
            Y.one('#quiz-time-left').setContent(M.util.get_string('timesup', 'finalquiz'));
        ...
        Y.one('#quiz-time-left').setContent(hours + ':' +
                M.mod_finalquiz.timer.two_digit(minutes) + ':' +
                M.mod_finalquiz.timer.two_digit(seconds));

        // Arrange for this method to be called again soon.
        M.mod_finalquiz.timer.timeoutid = setTimeout(M.mod_finalquiz.timer.update, 100);
    }
};

M.mod_finalquiz.nav = M.mod_finalquiz.nav || {};

M.mod_finalquiz.nav.update_flag_state = function(attemptid, questionid, newstate) {
    var Y = M.mod_finalquiz.nav.Y;
...
M.mod_finalquiz.nav.init = function(Y) {
    M.mod_finalquiz.nav.Y = Y;
    ...
    if (M.core_question_flags) {
        M.core_question_flags.add_listener(M.mod_finalquiz.nav.update_flag_state);
    }
};

M.mod_finalquiz.secure_window = {
    init: function(Y) {
        if (window.location.href.substring(0, 4) == 'file') {
            window.location = 'about:blank';
        }
        Y.delegate('contextmenu', M.mod_finalquiz.secure_window.prevent, document, '*');
        Y.delegate('mousedown',   M.mod_finalquiz.secure_window.prevent_mouse, 'body', '*');
        Y.delegate('mouseup',     M.mod_finalquiz.secure_window.prevent_mouse, 'body', '*');
        Y.delegate('dragstart',   M.mod_finalquiz.secure_window.prevent, document, '*');
        Y.delegate('selectstart', M.mod_finalquiz.secure_window.prevent_selection, document, '*');
        Y.delegate('cut',         M.mod_finalquiz.secure_window.prevent, document, '*');
        Y.delegate('copy',        M.mod_finalquiz.secure_window.prevent, document, '*');
        Y.delegate('paste',       M.mod_finalquiz.secure_window.prevent, document, '*');
        ...
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'press:67,86,88+ctrl');
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'up:67,86,88+ctrl');
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'down:67,86,88+ctrl');
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'press:67,86,88+meta');
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'up:67,86,88+meta');
        Y.on('key', M.mod_finalquiz.secure_window.prevent, '*', 'down:67,86,88+meta');
    },
    ...
    is_content_editable: function(n) {
        ...
        return M.mod_finalquiz.secure_window.is_content_editable(n);
    },
    ...
    prevent: function(e) {
        alert(M.util.get_string('functiondisabledbysecuremode', 'finalquiz'));
        e.halt();
    },
    ...
    prevent_mouse: function(e) {
        ...
        if (e.button == 1 && M.mod_finalquiz.secure_window.is_content_editable(e.target)) {
    ...
    init_close_button: function(Y, url) {
        Y.on('click', function(e) {
            M.mod_finalquiz.secure_window.close(url, 0)
        }, '#secureclosebutton');
    },
