...
require_once($CFG->dirroot.'/mod/finalquiz/override_form.php');
...
$url = new moodle_url('/mod/finalquiz/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/finalquiz:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'finalquiz'));
...
$sql = 'SELECT o.id
            FROM {finalquiz_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('finalquiz_overrides', 'id', array_keys($orphaned));
}
...
$table->head = array(
        $colname,
        get_string('overrides', 'finalquiz'),
        get_string('action'),
);
...
$overridedeleteurl = new moodle_url('/mod/finalquiz/overridedelete.php');
$overrideediturl = new moodle_url('/mod/finalquiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/finalquiz:attempt', $context, $override->userid)) {
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('quizopens', 'finalquiz');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'finalquiz');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('quizcloses', 'finalquiz');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'finalquiz');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'finalquiz');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'finalquiz');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'finalquiz');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'finalquiz');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'finalquiz') : get_string('none', 'finalquiz');
    }
...
echo html_writer::start_tag('div', array('id' => 'finalquizoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'finalquiz'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'finalquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'finalquiz'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the quiz.
    $users = get_users_by_capability($context, 'mod/finalquiz:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'finalquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'finalquiz'), 'get', $options);
}
