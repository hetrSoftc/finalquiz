...
$cm = get_coursemodule_from_id('finalquiz', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quiz = $DB->get_record('finalquiz', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = finalquiz_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see finalquiz reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/finalquiz/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = finalquiz_get_user_attempts($quiz->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new finalquiz_attempt($attempt, $quiz, $cm, $course, false);
...
redirect(new moodle_url('/mod/finalquiz/report.php', array(
