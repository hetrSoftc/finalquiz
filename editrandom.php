...
// Get the quiz slot.
$slot = $DB->get_record('finalquiz_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_finalquiz');
}

if (!$quiz = $DB->get_record('finalquiz', array('id' => $slot->quizid))) {
    print_error('invalidquizid', 'finalquiz');
}

$cm = get_coursemodule_from_instance('finalquiz', $slot->quizid, $quiz->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/finalquiz/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/finalquiz/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_finalquiz\form\randomquestion_form(new moodle_url('/mod/finalquiz/editrandom.php'),
...
$currentslottags = finalquiz_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the finalquiz_slots table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('finalquiz_slot_tags', "id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_finalquiz');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_finalquiz');
