...
/**
 * Implementaton of the finalquizaccess_timelimit plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquizaccess_timelimit extends finalquiz_access_rule_base {

    public static function make(finalquiz $quizobj, $timenow, $canignoretimelimits) {

        if (empty($quizobj->get_finalquiz()->timelimit) || $canignoretimelimits) {
            return null;
        }
    ...
    public function description() {
        return get_string('finalquiztimelimit', 'finalquizaccess_timelimit',
    ...
    public function add_preflight_check_form_fields(mod_finalquiz_preflight_check_form $quizform,
        ...
        $mform->addElement('header', 'honestycheckheader',
                get_string('confirmstartheader', 'finalquizaccess_timelimit'));
        $mform->addElement('static', 'honestycheckmessage', '',
                get_string('confirmstart', 'finalquizaccess_timelimit', format_time($this->quiz->timelimit)));
    }
}
