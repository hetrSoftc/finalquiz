...
/**
 * Implementaton of the finalquizaccess_openclosedate plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquizaccess_openclosedate extends quiz_access_rule_base {
    ...
    public function description() {
        ...
        if ($this->timenow < $this->quiz->timeopen) {
            $result[] = get_string('quiznotavailable', 'finalquizaccess_openclosedate',
                    userdate($this->quiz->timeopen));
            if ($this->quiz->timeclose) {
                $result[] = get_string('quizcloseson', 'finalquiz', userdate($this->quiz->timeclose));
            }

        } else if ($this->quiz->timeclose && $this->timenow > $this->quiz->timeclose) {
            $result[] = get_string('quizclosed', 'finalquiz', userdate($this->quiz->timeclose));

        } else {
            if ($this->quiz->timeopen) {
                $result[] = get_string('quizopenedon', 'finalquiz', userdate($this->quiz->timeopen));
            }
            if ($this->quiz->timeclose) {
                $result[] = get_string('quizcloseson', 'finalquiz', userdate($this->quiz->timeclose));
            }
    ...
    public function prevent_access() {
        $message = get_string('notavailable', 'finalquizaccess_openclosedate');
    ...
    public function time_left_display($attempt, $timenow) {
        ...
        // less than FINALQUIZ_SHOW_TIME_BEFORE_DEADLINE.
        $endtime = $this->end_time($attempt);
        if ($endtime !== false && $timenow > $endtime - FINALQUIZ_SHOW_TIME_BEFORE_DEADLINE) {
