...
/**
 * Implementaton of the finalquizaccess_safeexambrowser plugin.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquizaccess_safeexambrowser extends finalquiz_access_rule_base {
    ...
    /**
     * Add any fields that this rule requires to the finalquiz settings form. This
     * method is called from {@link mod_finalquiz_mod_form::definition()}, while the
    ...
    public static function add_settings_form_fields(
        ...
        $mform->addElement('textarea', 'safeexambrowser_allowedkeys',
                get_string('allowedbrowserkeys', 'finalquizaccess_safeexambrowser'),
        ...
        $mform->setAdvanced('safeexambrowser_allowedkeys',
                get_config('finalquizaccess_safeexambrowser', 'allowedkeys_adv')
        );
        $mform->addHelpButton('safeexambrowser_allowedkeys',
                'allowedbrowserkeys', 'finalquizaccess_safeexambrowser');
    }
    ...
    public static function save_settings($quiz) {
        ...
        if (empty($quiz->safeexambrowser_allowedkeys)) {
            $DB->delete_records('finalquizaccess_safeexambrowser', array('quizid' => $quiz->id));
        } else {
            $record = $DB->get_record('finalquizaccess_safeexambrowser', array('quizid' => $quiz->id));
            if (!$record) {
                $record = new stdClass();
                $record->quizid = $quiz->id;
                $record->allowedkeys = self::clean_keys($quiz->safeexambrowser_allowedkeys);
                $DB->insert_record('finalquizaccess_safeexambrowser', $record);
            } else {
                $record->allowedkeys = self::clean_keys($quiz->safeexambrowser_allowedkeys);
                $DB->update_record('finalquizaccess_safeexambrowser', $record);
            }
        }
    }
    ...
    public static function get_blocked_user_message() {
        $url = get_config('finalquizaccess_safeexambrowser', 'downloadlink');
        if ($url) {
            $a = new stdClass();
            $a->link = html_writer::link($url, $url);
            return get_string('safebrowsermustbeusedwithlink', 'finalquizaccess_safeexambrowser', $a);
        } else {
            return get_string('safebrowsermustbeused', 'finalquizaccess_safeexambrowser');
        }
    }
    ...
    public static function validate_keys($keys) {
        ...
        foreach ($keys as $i => $key) {
            if (!preg_match('~^[a-f0-9]{64}$~', $key)) {
                $errors[] = get_string('allowedbrowserkeyssyntax', 'finalquizaccess_safeexambrowser');
                break;
            }
        }
        if (count($keys) != count(array_unique($keys))) {
            $errors[] = get_string('allowedbrowserkeysdistinct', 'finalquizaccess_safeexambrowser');
        }
        return $errors;
    }
     ...
     * or of the user has the 'finalquizaccess/safeexambrowser:exemptfromcheck' capability.
     */
    public static function check_access(array $keys, context $context) {
        if (has_capability('finalquizaccess/safeexambrowser:exemptfromcheck', $context)) {
            return true;
        }
