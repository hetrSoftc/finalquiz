 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquizaccess_delaybetweenattempts extends finalquiz_access_rule_base {

    public static function make(finalquiz $quizobj, $timenow, $canignoretimelimits) {
        if (empty($quizobj->get_finalquiz()->delay1) && empty($quizobj->get_finalquiz()->delay2)) {
            return null;
        }
    ...
    public function prevent_new_attempt($numprevattempts, $lastattempt) {
        ...
        if ($this->timenow < $nextstarttime) {
            if ($this->quiz->timeclose == 0 || $nextstarttime <= $this->quiz->timeclose) {
                return get_string('youmustwait', 'finalquizaccess_delaybetweenattempts',
                        userdate($nextstarttime));
            } else {
                return get_string('youcannotwait', 'finalquizaccess_delaybetweenattempts');
            }
        }
        return false;
    }
