 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class finalfinalquiz_attempts_report extends finalquiz_default_report {
    ...
    /** @var object the finalquiz context. */
    protected $context;

    /** @var mod_finalfinalquiz_attempts_report_form The settings form to use. */
    protected $form;
    ...
    protected function init($mode, $formclass, $quiz, $cm, $course) {
        ...
        $this->qmsubselect = finalquiz_report_qm_filter_select($quiz);

        $this->form = new $formclass($this->get_base_url(),
                array('finalquiz' => $quiz, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/finalquiz:attempt', 'mod/finalquiz:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a final quiz report.
     *
     ...
     * @param mod_finalfinalquiz_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the final quiz.
    ...
    protected function print_standard_header_and_messages($cm, $course, $quiz,
        ...
        if ($strattemptnum = finalquiz_num_attempt_summary($quiz, $cm, true, $currentgroup)) {
            echo '<div class="finalquizattemptcounts">' . $strattemptnum . '</div>';
        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'finalquiz');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'finalquiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'finalquiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'finalquiz');
    }
     ...
     * @param mod_finalfinalquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_finalfinalquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/finalquiz:deleteattempts', $this->context);
    ...
    protected function delete_selected_attempts($quiz, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        ...
        foreach ($attemptids as $attemptid) {
            ...
            // Set the course module id before calling finalquiz_delete_attempt().
            $quiz->cmid = $cm->id;
