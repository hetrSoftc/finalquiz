...
/**
 * Helper functions for the final quiz reports.
 *
 ...
 */
function finalquiz_report_index_by_keys($datum, $keys, $keysunique = true) {
    if (!$datum) {
        return array();
    }
    ...
    if ($keys) {
        foreach ($datumkeyed as $datakey => $datakeyed) {
            $datumkeyed[$datakey] = finalquiz_report_index_by_keys($datakeyed, $keys, $keysunique);
        }
    }
    return $datumkeyed;
}

function finalquiz_report_unindex($datum) {
    if (!$datum) {
        return $datum;
    }
    $datumunkeyed = array();
    foreach ($datum as $value) {
        if (is_array($value)) {
            $datumunkeyed = array_merge($datumunkeyed, finalquiz_report_unindex($value));
        } else {
            $datumunkeyed[] = $value;
        }
    }
 ...
 */
function finalquiz_has_questions($quizid) {
    global $DB;
    return $DB->record_exists('finalquiz_slots', array('quizid' => $quizid));
}
 ...
 */
function finalquiz_report_get_significant_questions($quiz) {
    global $DB;
    ...
    $qsbyslot = $DB->get_records_sql("
            SELECT slot.slot,
              ...
              FROM {question} q
              JOIN {finalquiz_slots} slot ON slot.questionid = q.id
 ...
 */
function finalquiz_report_can_filter_only_graded($quiz) {
    return $quiz->attempts != 1 && $quiz->grademethod != FINALQUIZ_GRADEAVERAGE;
}
 ...
 *
 * @param object $quiz
 * @param string $quizattemptsalias sql alias for 'finalfinalquiz_attempts' table
 * @return string sql to test if this is an attempt that will contribute towards the grade of the user
 */
function finalquiz_report_qm_filter_select($quiz, $quizattemptsalias = 'quiza') {
    if ($quiz->attempts == 1) {
        // This quiz only allows one attempt.
        return '';
    }
    return finalquiz_report_grade_method_sql($quiz->grademethod, $quizattemptsalias);
}
 ...
 * empty string if the grading method is FINALQUIZ_GRADEAVERAGE and thus all attempts
 ...
 */
function finalquiz_report_grade_method_sql($grademethod, $quizattemptsalias = 'quiza') {
    switch ($grademethod) {
        case FINALQUIZ_GRADEHIGHEST :
            return "($quizattemptsalias.state = 'finished' AND NOT EXISTS (
                           SELECT 1 FROM {finalfinalquiz_attempts} qa2
 ...
 */
function finalquiz_report_grade_bands($bandwidth, $bands, $quizid, \core\dml\sql_join $usersjoins = null) {
    global $DB;
    if (!is_int($bands)) {
        debugging('$bands passed to finalquiz_report_grade_bands must be an integer. (' .
...
function finalquiz_report_highlighting_grading_method($quiz, $qmsubselect, $qmfilter) {
    if ($quiz->attempts == 1) {
        return '<p>' . get_string('onlyoneattemptallowed', 'finalquiz_overview') . '</p>';

    } else if (!$qmsubselect) {
        return '<p>' . get_string('allattemptscontributetograde', 'finalquiz_overview') . '</p>';

    } else if ($qmfilter) {
        return '<p>' . get_string('showinggraded', 'finalquiz_overview') . '</p>';

    } else {
        return '<p>' . get_string('showinggradedandungraded', 'finalquiz_overview',
                '<span class="gradedattempt">' . finalquiz_get_grading_option_name($quiz->grademethod) .
                '</span>') . '</p>';
    }
}
 ...
 */
function finalquiz_report_feedback_for_grade($grade, $quizid, $context) {
    global $DB;

    static $feedbackcache = array();

    if (!isset($feedbackcache[$quizid])) {
        $feedbackcache[$quizid] = $DB->get_records('finalquiz_feedback', array('quizid' => $quizid));
    }
    ...
    $feedbacktext = file_rewrite_pluginfile_urls($feedbacktext, 'pluginfile.php',
            $context->id, 'mod_finalquiz', 'feedback', $feedbackid);
 ...
 */
function finalquiz_report_scale_summarks_as_percentage($rawmark, $quiz, $round = true) {
    if ($quiz->sumgrades == 0) {
        return '';
    }
    ...
    if ($round) {
        $mark = finalquiz_format_grade($quiz, $mark);
    }
    return $mark . '%';
}
 ...
 */
function finalquiz_report_list($context) {
    global $DB;
    ...
    $reports = $DB->get_records('finalquiz_reports', null, 'displayorder DESC', 'name, capability');
    $reportdirs = core_component::get_plugin_list('finalquiz');
    ...
    foreach ($reportcaps as $name => $capability) {
        if (empty($capability)) {
            $capability = 'mod/finalquiz:viewreports';
        }
 ...
 */
function finalquiz_report_download_filename($report, $courseshortname, $quizname) {
 ...
 */
function finalquiz_report_default_report($context) {
    $reports = finalquiz_report_list($context);
 ...
 */
function finalquiz_no_questions_message($quiz, $cm, $context) {
    global $OUTPUT;

    $output = '';
    $output .= $OUTPUT->notification(get_string('noquestions', 'finalquiz'));
    if (has_capability('mod/finalquiz:manage', $context)) {
        $output .= $OUTPUT->single_button(new moodle_url('/mod/finalquiz/edit.php',
        array('cmid' => $cm->id)), get_string('editquiz', 'finalquiz'), 'get');
    }
 ...
 * @return bool
 */
function finalquiz_report_should_show_grades($quiz, context $context) {
    if ($quiz->timeclose && time() > $quiz->timeclose) {
        $when = mod_finalquiz_display_options::AFTER_CLOSE;
    } else {
        $when = mod_finalquiz_display_options::LATER_WHILE_OPEN;
    }
    $reviewoptions = mod_finalquiz_display_options::make_from_finalquiz($quiz, $when);

    return finalquiz_has_grades($quiz) &&
