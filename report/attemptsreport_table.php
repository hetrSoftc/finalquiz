...
/**
 * Base class for the table used by a {@link finalfinalquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class finalfinalquiz_attempts_report_table extends table_sql {
    ...
    /** @var object the final quiz settings for the final quiz we are reporting on. */
    protected $finalquiz;

    /** @var context the final quiz context. */
    protected $context;

    /** @var string HTML fragment to select the first/best/last attempt, if appropriate. */
    protected $qmsubselect;

    /** @var object mod_finalfinalquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this final quiz.. */
    protected $questions;
     ...
     * @param object $finalquiz
     * @param context $context
     * @param string $qmsubselect
     * @param mod_finalfinalquiz_attempts_report_options $options
    ...
    public function __construct($uniqueid, $quiz, $context, $qmsubselect,
            mod_finalfinalquiz_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/finalquiz/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'finalquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return finalquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = finalquiz_report_feedback_for_grade(
                finalquiz_rescale_grade($attempt->sumgrades, $this->finalquiz, false),
                $this->finalquiz->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/finalquiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'finalquiz')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // This part is the same for all cases. Join the users and finalquiz_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {finalquiz_attempts} quiza ON
        ...
        switch ($this->options->attempts) {
            case finalfinalquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'quiza.id IS NOT NULL AND quiza.preview = 0';
                break;
            case finalfinalquiz_attempts_report::ENROLLED_WITH:
                ...
                break;
            case finalfinalquiz_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "quiza.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case finalfinalquiz_attempts_report::ENROLLED_ALL:
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{finalfinalquiz_attempts} {$alias}quiza", "{$alias}quiza.uniqueid",
    ...
    public function wrap_html_finish() {
        ...
        echo '<a id="checkattempts" href="#">' .
                get_string('selectall', 'finalquiz') . '</a> / ';
        echo '<a id="uncheckattempts" href="#">' .
                get_string('selectnone', 'finalquiz') . '</a> ';
