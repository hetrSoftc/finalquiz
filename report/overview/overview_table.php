...
require_once($CFG->dirroot . '/mod/finalquiz/report/attemptsreport_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_overview_table extends finalquiz_attempts_report_table {
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(quizaouter.sumgrades) AS grade, COUNT(quizaouter.sumgrades) AS numaveraged
                  FROM {finalquiz_attempts} quizaouter
                  JOIN (
        ...
        if ($this->options->slotmarks) {
            ...
            $qubaids = new qubaid_join("{finalquiz_attempts} quizaouter
                  JOIN (
                       SELECT DISTINCT quiza.id
                         FROM $from
                        WHERE $where
    ...
    public function col_sumgrades($attempt) {
        ...
        return html_writer::link(new moodle_url('/mod/finalquiz/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'finalquiz')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/finalquiz/report/overview/report.php to see what fields are present,
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/finalquiz:regrade', $this->context)) {
