...
require_once($CFG->dirroot . '/mod/finalquiz/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_overview_report extends finalquiz_attempts_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/finalquiz:regrade', $this->context)) {
            ...
            if (!$table->is_downloading() && has_capability('mod/finalquiz:regrade', $this->context) &&
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header = get_string('qbrief', 'finalquiz', $question->number);
                    if (!$table->is_downloading()) {
                        $header .= '<br />';
                    } else {
                        $header .= ' ';
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($DB->record_exists('quiz_grades', array('finalquiz'=> $quiz->id))) {
    ...
    protected function start_regrade($quiz, $cm) {
        require_capability('mod/finalquiz:regrade', $this->context);
    ...
    protected function regrade_attempts_needing_it($quiz, \core\dml\sql_join $groupstudentsjoins) {
        ...
        $attempts = $DB->get_records_list('finalquiz_attempts', 'uniqueid',
