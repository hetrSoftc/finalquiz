...
/**
 * Base class for the options that control what is visible in an {@link finalfinalquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalfinalquiz_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the final quiz being reported on. */
    public $finalquiz;

    /** @var object the course module objects for the final quiz being reported on. */
    public $cm;
    ...
    protected static $statefields = array(
        'stateinprogress' => finalquiz_attempt::IN_PROGRESS,
        'stateoverdue'    => finalquiz_attempt::OVERDUE,
        'statefinished'   => finalquiz_attempt::FINISHED,
        'stateabandoned'  => finalquiz_attempt::ABANDONED,
    );

    /**
     * @var string finalfinalquiz_attempts_report::ALL_WITH or finalfinalquiz_attempts_report::ENROLLED_WITH
     *      finalfinalquiz_attempts_report::ENROLLED_WITHOUT or finalfinalquiz_attempts_report::ENROLLED_ALL
     */
    public $attempts = finalfinalquiz_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of finalquiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(finalquiz_attempt::IN_PROGRESS, finalquiz_attempt::OVERDUE,
            finalquiz_attempt::FINISHED, finalquiz_attempt::ABANDONED);
    ...
    public $pagesize = finalfinalquiz_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $finalquiz the settings for the quiz being reported on.
    ...
    public function __construct($mode, $quiz, $cm, $course) {
        ...
        $this->usercanseegrades = finalquiz_report_should_show_grades($quiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/finalquiz/report.php', $this->get_url_params());
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == finalquiz_attempts_report::ALL_WITH) {
                $this->attempts = finalquiz_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = finalquiz_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(finalquiz_attempts_report::ALL_WITH, finalquiz_attempts_report::ENROLLED_WITH,
                finalquiz_attempts_report::ENROLLED_WITHOUT, finalquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = finalquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if ($this->attempts == finalquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = finalquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
