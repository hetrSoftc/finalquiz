 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquiz_statistics_table extends flexible_table {
    /** @var object the finalquiz settings. */
    protected $quiz;
    ...
    public function __construct() {
        parent::__construct('mod-finalquiz-report-statistics-report');
    }
    ...
    public function statistics_setup($quiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'finalquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'finalquiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'finalquiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'finalquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'finalquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'finalquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'finalquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'finalquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'finalquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'finalquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'finalquiz_statistics');
    ...
    protected function col_actions($questionstat) {
        return finalquiz_question_action_icons($this->quiz, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'finalquiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'finalquiz_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'finalquiz_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'finalquiz_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'finalquiz_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the finalquiz. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return finalquiz_report_scale_summarks_as_percentage($questionstat->maxmark, $this->quiz);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'finalquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'finalquiz_statistics'),
     ...
     * other questions in this final quiz.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
