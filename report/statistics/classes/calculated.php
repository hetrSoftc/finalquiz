...
class calculated {
     ...
     */
    public function get_formatted_finalquiz_info_data($course, $cm, $quiz) {
        ...
        if ($quiz->timeopen) {
            $quizinfo[get_string('quizopen', 'finalquiz')] = userdate($quiz->timeopen);
        }
        if ($quiz->timeclose) {
            $quizinfo[get_string('quizclose', 'finalquiz')] = userdate($quiz->timeclose);
        }
