...
/**
 * SQL to fetch relevant 'finalfinalquiz_attempts' records.
 *
 * @param int    $quizid        final quiz id to get attempts for
 ...
 */
function finalquiz_statistics_attempts_sql($quizid, \core\dml\sql_join $groupstudentsjoins,
        $whichattempts = FINALQUIZ_GRADEAVERAGE, $includeungraded = false) {
    $fromqa = "{finalfinalquiz_attempts} quiza ";
    $whereqa = 'quiza.quiz = :quizid AND quiza.preview = 0 AND quiza.state = :quizstatefinished';
    $qaparams = array('quizid' => (int)$quizid, 'finalquizstatefinished' => finalquiz_attempt::FINISHED);
    ...
    $whichattemptsql = finalquiz_report_grade_method_sql($whichattempts);
    if ($whichattemptsql) {
...
/**
 * Return a {@link qubaid_condition} from the values returned by {@link finalquiz_statistics_attempts_sql}.
 *
 * @param int     $quizid
 ...
 * @return        \qubaid_join
 */
function finalquiz_statistics_qubaids_condition($quizid, \core\dml\sql_join $groupstudentsjoins,
        $whichattempts = FINALQUIZ_GRADEAVERAGE, $includeungraded = false) {
 ...
 */
function finalquiz_statistics_graph_get_new_colour() {
    debugging('The function finalquiz_statistics_graph_get_new_colour() is deprecated, please do not use it any more. '
