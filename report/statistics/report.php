...
require_once($CFG->dirroot . '/mod/finalquiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/finalquiz/report/statistics/statisticslib.php');
/**
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquiz_statistics_report extends finalquiz_default_report {

    /** @var context_module context of this quiz.*/
    protected $context;

    /** @var finalquiz_statistics_table instance of table class used for main questions stats table. */
    protected $table;
    ...
    public function display($quiz, $cm, $course) {
        ...
        if (!finalquiz_has_questions($quiz->id)) {
            $this->print_header_and_tabs($cm, $course, $quiz, 'statistics');
            echo finalquiz_no_questions_message($quiz, $cm, $this->context);
            return true;
        }
        ...
        $reporturl = new moodle_url('/mod/finalquiz/report.php', $pageoptions);

        $mform = new finalquiz_statistics_settings_form($reporturl, compact('finalquiz'));
        ...
        if (empty($currentgroup)) {
            ...
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/finalquiz:reviewmyattempts', 'mod/finalquiz:attempt'), $currentgroup);
        ...
        $qubaids = finalquiz_statistics_qubaids_condition($quiz->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new finalquiz_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'finalquiz_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'finalquiz_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = finalquiz_report_download_filename($report, $courseshortname, $quiz->name);
        $this->table->is_downloading($download, $filename,
                get_string('finalquizstructureanalysis', 'finalquiz_statistics'));
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'finalquiz_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $quizstats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'finalquiz_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $quizinfo = $quizstats->get_formatted_finalquiz_info_data($course, $cm, $quiz);
            $this->download_finalquiz_info_table($quizinfo);

            if ($quizstats->s()) {
                $this->output_finalquiz_structure_analysis_table($questionstats);
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtoquizreport', 'finalquiz_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'finalquiz_statistics', $number), 3);
                }
            ...
            echo $OUTPUT->heading(get_string('quizinformation', 'finalquiz_statistics'), 3);
            ...
            $quizinfo = $quizstats->get_formatted_finalquiz_info_data($course, $cm, $quiz);
            echo $this->output_finalquiz_info_table($quizinfo);
            if ($quizstats->s()) {
                echo $OUTPUT->heading(get_string('quizstructureanalysis', 'finalquiz_statistics'), 3);
                $this->output_finalquiz_structure_analysis_table($questionstats);
    ...
    protected function output_individual_question_data($quiz, $questionstat) {
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'finalquiz'), $quiz->name);
        $questioninfotable->data[] = array(get_string('questionname', 'finalquiz_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'finalquiz_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'finalquiz_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'finalquiz_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'finalquiz_statistics'),
            'facility' => get_string('facility', 'finalquiz_statistics'),
            'sd' => get_string('standarddeviationq', 'finalquiz_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'finalquiz_statistics'),
            'intended_weight' => get_string('intended_weight', 'finalquiz_statistics'),
            'effective_weight' => get_string('effective_weight', 'finalquiz_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'finalquiz_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'finalquiz_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'finalquiz_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'finalquiz_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new finalquiz_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'finalquiz_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'finalquiz_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'finalquiz_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'finalquiz_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'finalquiz_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'finalquiz_statistics', $questiontabletitle);
            }
     ...
     *                                                                                               variants.
     */
    protected function output_finalquiz_structure_analysis_table($questionstats) {
        $tooutput = array();
     ...
     *
     * @param array $quizinfo as returned by {@link get_formatted_finalquiz_info_data()}.
     * @return string the HTML.
     */
    protected function output_finalquiz_info_table($quizinfo) {
     ...
     * @param array $quizinfo as returned by {@link get_formatted_finalquiz_info_data()}.
     */
    protected function download_finalquiz_info_table($quizinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('quizinformation', 'finalquiz_statistics'));
    ...
    protected function output_statistics_graph($quizorid, $qubaids) {
        ...
        $quiz = $quizorid;
        if (!is_object($quiz)) {
            $quiz = $DB->get_record('finalquiz', array('id' => $quizorid), '*', MUST_EXIST);
        }
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'finalquiz_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'finalquiz_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'finalquiz_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'finalquiz_statistics');
     ...
     *                                   $quiz->grademethod ie.
     *                                   FINALQUIZ_GRADEAVERAGE, FINALQUIZ_GRADEHIGHEST, FINALQUIZ_ATTEMPTLAST or FINALQUIZ_ATTEMPTFIRST
    ...
    public function get_all_stats_and_analysis(
        ...
        $quizcalc = new \finalquiz_statistics\calculator($progress);
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'finalquiz_statistics'));
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'finalquiz_statistics'),
    ...
    protected function output_caching_info($lastcachetime, $quizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        ...
        list($fromqa, $whereqa, $qaparams) = finalquiz_statistics_attempts_sql($quizid, $groupstudentsjoins, $whichattempts, true);
        ...
        $output .= get_string('lastcalculated', 'finalquiz_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'finalquiz_statistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('finalquiz_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    public function load_and_initialise_questions_for_calculations($quiz) {
        // Load the questions.
        $questions = finalquiz_report_get_significant_questions($quiz);
        $questionids = array();
        foreach ($questions as $question) {
