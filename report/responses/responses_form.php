...
require_once($CFG->dirroot . '/mod/finalquiz/report/attemptsreport_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quiz_responses_settings_form extends mod_finalfinalquiz_attempts_report_form {
    ...
    protected function other_preference_fields(MoodleQuickForm $mform) {
        ...
        $mform->disabledIf('qtext', 'attempts', 'eq', finalquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('resp',  'attempts', 'eq', finalquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('right', 'attempts', 'eq', finalquiz_attempts_report::ENROLLED_WITHOUT);
    }

    public function validation($data, $files) {
        $errors = parent::validation($data, $files);

        if ($data['attempts'] != finalquiz_attempts_report::ENROLLED_WITHOUT && !(
                $data['qtext'] || $data['resp'] || $data['right'])) {
            $errors['coloptions'] = get_string('reportmustselectstate', 'finalquiz');
        }
    ...
    protected function other_attempt_fields(MoodleQuickForm $mform) {
        ...
        if (quiz_allows_multiple_tries($this->_customdata['finalquiz'])) {
            ...
            $mform->disabledIf('whichtries', 'attempts', 'eq', finalquiz_attempts_report::ENROLLED_WITHOUT);
        }
    }
}
