...
require_once($CFG->dirroot . '/mod/finalquiz/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class finalquiz_grading_report extends finalquiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        require_capability('mod/finalquiz:grade', $this->context);
        $shownames = has_capability('finalquiz/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('finalquiz/grading:viewidnumber', $this->context);
        ...
        $this->questions = finalquiz_report_get_significant_questions($quiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'finalquiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/finalquiz:reviewmyattempts', 'mod/finalquiz:attempt'), $this->currentgroup);
        }

        $hasquestions = finalquiz_has_questions($quiz->id);
        $counts = null;
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'finalquiz_grading'));
            }
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $params = array('mangrquizid' => $this->cm->instance, 'statefinished' => finalquiz_attempt::FINISHED);
        ...
        return new qubaid_join("{finalfinalquiz_attempts} quiza $usersjoin ", 'quiza.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = finalquiz_attempt::FINISHED;
        $params[] = $this->quiz->id;
    ...
    protected function base_url() {
        return new moodle_url('/mod/finalquiz/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'finalquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'finalquiz_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'finalquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'finalquiz_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'finalquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'finalquiz_grading');
        $table->head[] = get_string('qtypeveryshort', 'question');
        $table->head[] = get_string('questionname', 'finalquiz_grading');
        $table->head[] = get_string('tograde', 'finalquiz_grading');
        $table->head[] = get_string('alreadygraded', 'finalquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'finalquiz_grading');
        }
        $table->head[] = get_string('total', 'finalquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new finalquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'finalquiz_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'finalquiz_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'finalquiz_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = finalquiz_get_review_options($this->quiz, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'class' => 'btn btn-primary', 'value' => get_string('saveandnext', 'finalquiz_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'finalquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'finalquiz_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new finalquiz_attempt($attempt, $this->quiz, $this->cm, $this->course);
            ...
            $events[] = \mod_finalquiz\event\question_manually_graded::create($params);
        }
