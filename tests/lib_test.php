...
/**
 * Unit tests for (some of) mod/finalquiz/locallib.php.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_finalquiz_lib_testcase extends advanced_testcase {
    public function test_finalquiz_has_grades() {
        $quiz = new stdClass();
        $quiz->grade = '100.0000';
    ...
    public function test_finalquiz_format_grade() {
        $quiz = new stdClass();
    ...
    public function test_finalquiz_get_grade_format() {
        $quiz = new stdClass();
        $quiz->decimalpoints = 2;
    ...
    public function test_finalquiz_format_question_grade() {
        $quiz = new stdClass();
        $quiz->decimalpoints = 2;
        $quiz->questiondecimalpoints = 2;
     ...
     */
    public function test_finalquiz_delete_instance() {
        global $SITE, $DB;
        ...
        quiz_add_finalquiz_question($standardq->id, $quiz);
        ...
        $count = $DB->count_records('finalquiz', array('id' => $quiz->id));
        $this->assertEquals(0, $count);
    }
     ...
     */
    public function test_finalquiz_get_completion_state() {
        global $CFG, $DB;
        $this->resetAfterTest(true);
        ...
        $cm = get_coursemodule_from_id('finalquiz', $quiz->cmid);
    ...
    public function test_finalquiz_get_user_attempts() {
        global $DB;
        $this->resetAfterTest();

        $dg = $this->getDataGenerator();
        $quizgen = $dg->get_plugin_generator('mod_finalquiz');
        ...
        quiz_add_finalquiz_question($question->id, $quiz1);
        quiz_add_finalquiz_question($question->id, $quiz2);
        ...
        // Set attempts.
        $quba1a = question_engine::make_questions_usage_by_activity('mod_finalquiz', $quizobj1a->get_context());
        $quba1a->set_preferred_behaviour($quizobj1a->get_finalquiz()->preferredbehaviour);
        $quba1b = question_engine::make_questions_usage_by_activity('mod_finalquiz', $quizobj1b->get_context());
        $quba1b->set_preferred_behaviour($quizobj1b->get_finalquiz()->preferredbehaviour);
        $quba1c = question_engine::make_questions_usage_by_activity('mod_finalquiz', $quizobj1c->get_context());
        $quba1c->set_preferred_behaviour($quizobj1c->get_finalquiz()->preferredbehaviour);
        $quba1d = question_engine::make_questions_usage_by_activity('mod_finalquiz', $quizobj1d->get_context());
        $quba1d->set_preferred_behaviour($quizobj1d->get_finalquiz()->preferredbehaviour);
        $quba2a = question_engine::make_questions_usage_by_activity('mod_finalquiz', $quizobj2a->get_context());
        $quba2a->set_preferred_behaviour($quizobj2a->get_finalquiz()->preferredbehaviour);
     ...
     */
    public function test_finalquiz_get_group_override_priorities() {
        global $DB;
        $this->resetAfterTest();
    ...
    public function test_finalquiz_core_calendar_provide_event_action_open() {
        $this->resetAfterTest();
        ...
        // Create a quiz.
        $quiz = $this->getDataGenerator()->create_module('finalquiz', array('course' => $course->id,
            'timeopen' => time() - DAYSECS, 'timeclose' => time() + DAYSECS));

        // Create a calendar event.
        $event = $this->create_action_event($course->id, $quiz->id, FINALQUIZ_EVENT_TYPE_OPEN);

        // Create an action factory.
        $factory = new \core_calendar\action_factory();

        // Decorate action event.
        $actionevent = mod_finalquiz_core_calendar_provide_event_action($event, $factory);

        // Confirm the event was decorated.
        $this->assertInstanceOf('\core_calendar\local\event\value_objects\action', $actionevent);
        $this->assertEquals(get_string('attemptquiznow', 'finalquiz'), $actionevent->get_name());
        $this->assertInstanceOf('moodle_url', $actionevent->get_url());
        $this->assertEquals(1, $actionevent->get_item_count());
        $this->assertTrue($actionevent->is_actionable());
    }

    public function test_finalquiz_core_calendar_provide_event_action_closed() {
        $this->resetAfterTest();
        ...
        // Create a calendar event.
        $event = $this->create_action_event($course->id, $quiz->id, FINALQUIZ_EVENT_TYPE_CLOSE);

        // Create an action factory.
        $factory = new \core_calendar\action_factory();

        // Confirm the result was null.
        $this->assertNull(mod_finalquiz_core_calendar_provide_event_action($event, $factory));
    }

    public function test_finalquiz_core_calendar_provide_event_action_open_in_future() {
        $this->resetAfterTest();
    ...
    public function test_finalquiz_core_calendar_provide_event_action_no_capability() {
        global $DB;
        ...
        assign_capability('mod/finalquiz:reviewmyattempts', CAP_PROHIBIT, $studentrole->id, $coursecontext);
        assign_capability('mod/finalquiz:attempt', CAP_PROHIBIT, $studentrole->id, $coursecontext);
    ...
    public function test_finalquiz_core_calendar_provide_event_action_already_finished() {
        global $DB;
     ...
     * @param string $eventtype The event type. eg. FINALQUIZ_EVENT_TYPE_OPEN.
     ...
     */
    public function test_mod_finalquiz_completion_get_active_rule_descriptions() {
        $this->resetAfterTest();
        $this->setAdminUser();

        // Two activities, both with automatic completion. One has the 'completionsubmit' rule, one doesn't.
        $course = $this->getDataGenerator()->create_course(['enablecompletion' => 2]);
        $quiz1 = $this->getDataGenerator()->create_module('finalquiz', [
            'course' => $course->id,
            'completion' => 2,
            'completionattemptsexhausted' => 1,
            'completionpass' => 1
        ]);
        $quiz2 = $this->getDataGenerator()->create_module('finalquiz', [
            'course' => $course->id,
            'completion' => 2,
            'completionattemptsexhausted' => 0,
            'completionpass' => 0
        ]);
        $cm1 = cm_info::create(get_coursemodule_from_instance('finalquiz', $quiz1->id));
        $cm2 = cm_info::create(get_coursemodule_from_instance('finalquiz', $quiz2->id));
        ...
        $activeruledescriptions = [
            get_string('completionattemptsexhausteddesc', 'finalquiz'),
            get_string('completionpassdesc', 'finalquiz'),
        ];
        $this->assertEquals(mod_finalquiz_get_completion_active_rule_descriptions($cm1), $activeruledescriptions);
        $this->assertEquals(mod_finalquiz_get_completion_active_rule_descriptions($cm2), []);
        $this->assertEquals(mod_finalquiz_get_completion_active_rule_descriptions($moddefaults), $activeruledescriptions);
        $this->assertEquals(mod_finalquiz_get_completion_active_rule_descriptions(new stdClass()), []);
    }
    ...
    public function test_creation_with_no_calendar_capabilities() {
        ...
        $generator = self::getDataGenerator()->get_plugin_generator('mod_finalquiz');
