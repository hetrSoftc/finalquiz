 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_finalquiz_calendar_event_modified_testcase extends advanced_testcase {
     ...
     */
    protected function create_finalquiz_instance(array $properties) {
        global $DB;
     ...
     */
    protected function create_finalquiz_calendar_event(\stdClass $quiz, array $eventproperties) {
        $defaultproperties = [
            'name' => 'Test event',
            'description' => '',
            'format' => 1,
            ...
            'instance' => $quiz->id,
            'eventtype' => FINALQUIZ_EVENT_TYPE_OPEN,
            'timestart' => time(),
            'timeduration' => 86400,
            'visible' => 1
        ];
     ...
     */
    public function test_mod_finalquiz_core_calendar_event_timestart_updated_unknown_event() {
        global $DB;
        ...
        $quiz = $this->create_finalquiz_instance(['timeopen' => $timeopen, 'timeclose' => $timeclose]);
        $event = $this->create_finalquiz_calendar_event($quiz, [
            'eventtype' => FINALQUIZ_EVENT_TYPE_OPEN . "SOMETHING ELSE",
            'timestart' => 1
        ]);

        mod_finalquiz_core_calendar_event_timestart_updated($event, $quiz);

        $quiz = $DB->get_record('finalquiz', ['id' => $quiz->id]);
        $this->assertEquals($timeopen, $quiz->timeopen);
        $this->assertEquals($timeclose, $quiz->timeclose);
    }

    /**
     * A FINALQUIZ_EVENT_TYPE_OPEN event should update the timeopen property of
     * the quiz activity.
     */
    public function test_mod_finalquiz_core_calendar_event_timestart_updated_open_event() {
        global $DB;
        ...
        $quiz = $this->create_finalquiz_instance([
            'timeopen' => $timeopen,
            'timeclose' => $timeclose,
    ...
    /**
     * A FINALQUIZ_EVENT_TYPE_CLOSE event should update the timeclose property of
     * the quiz activity.
     */
    public function test_mod_finalquiz_core_calendar_event_timestart_updated_close_event() {
        global $DB;
        ...
        $event = $this->create_finalquiz_calendar_event($quiz, [
            'eventtype' => FINALQUIZ_EVENT_TYPE_CLOSE,
    ...
    /**
     * A FINALQUIZ_EVENT_TYPE_OPEN event should not update the timeopen property of
     * the quiz activity if it's an override.
     */
    public function test_mod_finalquiz_core_calendar_event_timestart_updated_open_event_override() {
        global $DB;
     ...
     */
    public function test_student_role_cant_update_finalquiz_activity() {
        global $DB;
        ...
        $newquiz = $DB->get_record('finalquiz', ['id' => $quiz->id]);
     ...
     * event.
     */
    public function test_teacher_role_can_update_finalquiz_activity() {
        global $DB;
     ...
     */
    public function test_mod_finalquiz_core_calendar_get_valid_event_timestart_range_unknown_event() {
        global $DB;
        ...
        list ($min, $max) = mod_finalquiz_core_calendar_get_valid_event_timestart_range($event, $quiz);
        $this->assertNull($min);
        $this->assertNull($max);
    }
     ...
     */
    public function test_mod_finalquiz_core_calendar_get_valid_event_timestart_range_open_event() {
        global $DB;
     ...
     */
    public function test_mod_finalquiz_core_calendar_get_valid_event_timestart_range_override_event() {
        global $DB;
     ...
     */
    public function test_mod_finalquiz_core_calendar_get_valid_event_timestart_range_close_event() {
        global $DB;
     ...
     */
    public function test_core_calendar_event_timestart_updated_update_finalquiz_attempt() {
        global $DB;
        ...
        $quiz = $DB->get_record('finalquiz', ['id' => $quiz->id]);
        $attempt = $DB->get_record('finalquiz_attempts', ['id' => $attemptid]);
