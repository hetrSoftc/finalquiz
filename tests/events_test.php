 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_events_testcase extends advanced_testcase {
     ...
     */
    protected function prepare_finalquiz_data($ispreview = false) {
        ...
        $cm = get_coursemodule_from_instance('finalquiz', $quiz->id, $course->id);
    ...
    public function test_attempt_submitted() {
        ...
        list($quizobj, $quba, $attempt) = $this->prepare_finalquiz_data();
        ...
        $event = $events[2];
        $this->assertInstanceOf('\mod_finalquiz\event\attempt_submitted', $event);
        $this->assertEquals('finalquiz_attempts', $event->objecttable);
        ...
        $legacydata->component = 'mod_finalquiz';
    ...
    public function test_attempt_becameoverdue() {
        ...
        $event = $events[0];
        $this->assertInstanceOf('\mod_finalquiz\event\attempt_becameoverdue', $event);
        $this->assertEquals('finalquiz_attempts', $event->objecttable);
        $this->assertEquals($quizobj->get_context(), $event->get_context());
        $this->assertEquals($attempt->userid, $event->relateduserid);
        ...
        $this->assertEquals('finalquiz_attempt_overdue', $event->get_legacy_eventname());
        $legacydata = new stdClass();
        $legacydata->component = 'mod_finalquiz';
        $legacydata->attemptid = (string) $attempt->id;
        $legacydata->timestamp = $timefinish;
    ...
    public function test_attempt_abandoned() {
        ...
        $this->assertCount(1, $events);
        $event = $events[0];
        $this->assertInstanceOf('\mod_finalquiz\event\attempt_abandoned', $event);
        $this->assertEquals('finalquiz_attempts', $event->objecttable);
        $this->assertEquals($quizobj->get_context(), $event->get_context());
        $this->assertEquals($attempt->userid, $event->relateduserid);
        // Submitterid should be the user, but as we are in PHP Unit, CLI_SCRIPT is set to true which sets null in submitterid.
    ...
    public function test_attempt_started() {
        ...
        $this->assertInstanceOf('\mod_finalquiz\event\attempt_started', $event);
        $this->assertEquals('finalquiz_attempts', $event->objecttable);
    ...
    public function test_edit_page_viewed() {
        ...
        $event = \mod_finalquiz\event\edit_page_viewed::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\edit_page_viewed', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'editquestions', 'view.php?id=' . $quiz->cmid, $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_deleted() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\attempt_deleted', $event);
        $this->assertEquals(context_module::instance($quizobj->get_cmid()), $event->get_context());
        $expected = array($quizobj->get_courseid(), 'finalquiz', 'delete attempt', 'report.php?id=' . $quizobj->get_cmid(),
    ...
    public function test_preview_attempt_deleted() {
        ...
        list($quizobj, $quba, $previewattempt) = $this->prepare_finalquiz_data(true);
    ...
    public function test_report_viewed() {
        ...
        $event = \mod_finalquiz\event\report_viewed::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\report_viewed', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'report', 'report.php?id=' . $quiz->cmid . '&mode=overview',
            $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_reviewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'review', 'review.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_attempt_summary_viewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'view summary', 'summary.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_created() {
        ...
        $event = \mod_finalquiz\event\user_override_created::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\user_override_created', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_created() {
        ...
        $event = \mod_finalquiz\event\group_override_created::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\group_override_created', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_user_override_updated() {
        ...
        $event = \mod_finalquiz\event\user_override_updated::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\user_override_updated', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'edit override', 'overrideedit.php?id=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_updated() {
        ...
        $event = \mod_finalquiz\event\group_override_updated::create($params);
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\group_override_updated', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'edit override', 'overrideedit.php?id=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_override_deleted() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\user_override_deleted', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'delete override', 'overrides.php?cmid=' . $quiz->cmid, $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_group_override_deleted() {
        ...
        // Check that the event data is valid.
        $this->assertInstanceOf('\mod_finalquiz\event\group_override_deleted', $event);
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'delete override', 'overrides.php?cmid=' . $quiz->cmid, $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_attempt_viewed() {
        ...
        $this->assertEquals(context_module::instance($quiz->cmid), $event->get_context());
        $expected = array($course->id, 'finalquiz', 'continue attempt', 'review.php?attempt=1', $quiz->id, $quiz->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_question_manually_graded() {
        ...
        $event = \mod_finalquiz\event\question_manually_graded::create($params);
