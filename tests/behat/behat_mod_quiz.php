...
/**
 * Steps definitions related to mod_finalquiz.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_finalquiz extends behat_question_base {
    ...
    public function quiz_contains_the_following_questions($quizname, TableNode $data) {
        ...
        $quiz = $DB->get_record('finalquiz', array('name' => $quizname), '*', MUST_EXIST);
        ...
        foreach ($data->getHash() as $questiondata) {
            ...
            if ($question->qtype == 'random') {
                ...
                // Add the question.
                quiz_add_finalquiz_question($question->id, $quiz, $page, $maxmark);
            }

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('finalquiz_slots', 'MAX(slot)', array('quizid' => $quiz->id));
                    $DB->set_field('finalquiz_slots', 'requireprevious', 1,
    ...
    public function quiz_contains_the_following_sections($quizname, TableNode $data) {
        ...
        if ($section->firstslot > $DB->count_records('finalquiz_slots', array('quizid' => $quiz->id))) {
     ...
     */
    public function i_add_question_to_the_finalquiz_with($questiontype, $quizname, TableNode $questiondata) {
        $quizname = $this->escape($quizname);
        $editquiz = $this->escape(get_string('editquiz', 'finalquiz'));
        $quizadmin = $this->escape(get_string('pluginadministration', 'finalquiz'));
        $addaquestion = $this->escape(get_string('addaquestion', 'finalquiz'));
     ...
     */
    public function i_set_the_max_mark_for_finalquiz_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'finalquiz')));
     ...
     */
    public function i_open_the_add_to_finalquiz_menu_for($pageorlast) {
     ...
     */
    public function i_should_see_on_finalquiz_page($questionname, $pagenumber) {
     ...
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_finalquiz_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
     ...
     */
    public function i_should_see_before_on_the_edit_finalquiz_page($firstquestionname, $secondquestionname) {
     ...
     */
    public function should_have_number_on_the_edit_finalquiz_page($questionname, $number) {
     ...
     */
    public function i_should_see_question_in_section_in_the_finalquiz_navigation($questionnumber, $sectionheading) {
        ...
        $xpath = "//div[@id = 'mod_finalquiz_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
