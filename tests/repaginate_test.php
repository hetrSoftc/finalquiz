...
/**
 * Unit tests for the {@link \mod_finalquiz\repaginate} class.
...
require_once($CFG->dirroot . '/mod/finalquiz/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_finalquiz\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_repaginate_testable extends \mod_finalquiz\repaginate {
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $quizslots;
    /** @var mod_finalquiz_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_finalquiz_slots($this->get_finalquiz_object()->get_slots());
        $this->repaginate = new mod_finalquiz_repaginate_testable(0, $this->quizslots);
    }
     ...
     */
    private function get_finalquiz_object() {
        global $SITE;
        ...
        $cm = get_coursemodule_from_instance('finalquiz', $quiz->id, $SITE->id);
        ...
        quiz_add_finalquiz_question($shortanswer->id, $quiz);
        quiz_add_finalquiz_question($numerical->id, $quiz);
        quiz_add_finalquiz_question($essay->id, $quiz);
        quiz_add_finalquiz_question($truefalse->id, $quiz);
        quiz_add_finalquiz_question($match->id, $quiz);

        // Return the quiz object.
        $quizobj = new quiz($quiz, $cm, $SITE);
        return \mod_finalquiz\structure::create_for_quiz($quizobj);
    }
     ...
     * @param string $slots
     */
    private function set_finalquiz_slots($slots = null) {
        if (!$slots) {
            $this->quizslots = $this->get_finalquiz_object()->get_slots();
        } else {
    ...
    public function test_repaginate_the_rest() {
        $this->set_finalquiz_slots();
        $slotfrom = 1;
        $type = \mod_finalquiz\repaginate::LINK;
        $expected = array();
        foreach ($this->quizslots as $slot) {
        ...
        $type = \mod_finalquiz\repaginate::UNLINK;
        $expected = array();
        foreach ($this->quizslots as $slot) {
