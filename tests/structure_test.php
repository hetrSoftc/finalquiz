 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty quiz.
     * @return array with three elements quiz, cm and course.
     */
    protected function prepare_finalquiz_data() {
    ...
    protected function create_test_quiz($layout) {
        list($quiz, $cm, $course) = $this->prepare_finalquiz_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                quiz_add_finalquiz_question($q->id, $quiz, $page);
                $lastpage = $page;
            }
        }
     ...
     * @param \mod_finalquiz\structure $structure the structure to test.
     */
    protected function assert_finalquiz_layout($expectedlayout, \mod_finalquiz\structure $structure) {
        $sections = $structure->get_sections();
     ...
     * used by create_test_quiz as assert_finalquiz_layout.
    ...
    public function test_get_finalquiz_slots() {
    ...
    public function test_finalquiz_has_one_section_by_default() {
        $quizobj = $this->create_test_quiz(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_finalquiz\structure::create_for_quiz($quizobj);
    ...
    public function test_finalquiz_remove_slot() {
        $quizobj = $this->create_test_quiz(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_finalquiz_removing_a_random_question_deletes_the_question() {
        global $DB;
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_finalquiz\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_finalquiz\repaginate::UNLINK);
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $structure = \mod_finalquiz\structure::create_for_quiz($quiz);
