...
/**
 * Unit tests for (some of) mod/finalquiz/report/reportlib.php
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_finalquiz_reportlib_testcase extends advanced_testcase {
    public function test_finalquiz_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
    ...
    public function test_finalquiz_report_scale_summarks_as_percentage() {
        $quiz = new stdClass();
        $quiz->sumgrades = 10;
    ...
    public function test_finalquiz_report_qm_filter_select_only_one_attempt_allowed() {
        $quiz = new stdClass();
        $quiz->attempts = 1;
        $this->assertSame('', quiz_report_qm_filter_select($quiz));
    }

    public function test_finalquiz_report_qm_filter_select_average() {
        $quiz = new stdClass();
        $quiz->attempts = 10;
        $quiz->grademethod = FINALQUIZ_GRADEAVERAGE;
        $this->assertSame('', quiz_report_qm_filter_select($quiz));
    }

    public function test_finalquiz_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();
        ...
        $DB->insert_record('finalquiz_attempts', $fakeattempt);
        ...
        $quiz->grademethod = FINALQUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {finalquiz_attempts} quiza WHERE userid = ? AND quiz = ? AND "
        ...
        $quiz->grademethod = FINALQUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {finalquiz_attempts} qa_alias WHERE userid = ? AND quiz = ? AND "
