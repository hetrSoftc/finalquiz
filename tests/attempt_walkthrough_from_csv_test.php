 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_finalquiz_attempt_walkthrough_from_csv_testcase extends advanced_testcase {
    ...
    public function create_quiz($quizsettings, $qs) {
        ...
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                quiz_add_finalquiz_question($slotquestion['id'], $this->quiz, 0, $slotquestion['mark']);
            } else {
     ...
     */
    protected function create_finalquiz_simulate_attempts_and_check_results($quizsettings, $csvdata) {
