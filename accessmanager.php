 ...
 * final quiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
...
class quiz_access_manager {
    /** @var finalquiz the final quiz settings object. */
    protected $quizobj;
    ...
    /**
     * Create an instance for a particular final quiz.
     * @param object $quizobj An instance of the class final quiz from attemptlib.php.
     *      The final quiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/finalquiz:ignoretimelimits', ...)).
     */
     ...
     * @param finalquiz $quizobj information about the quiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/finalquiz:ignoretimelimits capability.
     * @return array of {@link finalquiz_access_rule_base}s.
     */
    protected function make_rules($quizobj, $timenow, $canignoretimelimits) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['finalquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('finalquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the finalquiz table.
     *
     * @param mod_finalquiz_mod_form $quizform the finalquiz settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_finalquiz_mod_form $quizform, MoodleQuickForm $mform) {
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'finalquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_finalquiz_mod_form $quizform the quiz form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_finalquiz_mod_form $quizform) {
     ...
     *
     * @param int $quizid the final quiz id.
     * @return object mdl_finalquiz row with extra fields.
     */
    public static function load_finalquiz_and_settings($quizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizid, $rules, 'finalquiz.*');
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_finalquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_finalquiz_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_finalquiz_preflight_check_form($url->out_omit_querystring(),
     ...
     * final quiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant finalfinalquiz_attempts row.
    ...
    /**
     * @return bolean if this final quiz should only be shown to students in a popup window.
     */
     ...
     *
     * @param mod_finalquiz_renderer $output the final quiz renderer.
    ...
    /**
     * Make some text into a link to review the final quiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(finalquiz_attempt::IN_PROGRESS, finalquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = finalquiz_attempt_state($this->quizobj->get_finalquiz(), $attempt);
        $reviewoptions = mod_finalquiz_display_options::make_from_finalquiz(
                $this->quizobj->get_finalquiz(), $when);
