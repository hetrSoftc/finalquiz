...
if (!$quizobj->has_questions()) {
    if ($quizobj->has_capability('mod/finalquiz:manage')) {
        redirect($quizobj->edit_url());
    } else {
        print_error('cannotstartnoquestions', 'finalquiz', $quizobj->view_url());
    }
}
...
list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
    finalquiz_validate_new_attempt($quizobj, $accessmanager, $forcenew, $page, true);

// Check access.
if (!$quizobj->is_preview_user() && $messages) {
    $output = $PAGE->get_renderer('mod_finalquiz');
    print_error('attempterror', 'finalquiz', $quizobj->view_url(),
...
if ($accessmanager->is_preflight_check_required($currentattemptid)) {
    ...
    if ($mform->is_cancelled()) {
        ...
        $PAGE->set_title($quizobj->get_finalquiz_name());
        $accessmanager->setup_attempt_page($PAGE);
        $output = $PAGE->get_renderer('mod_finalquiz');
        if (empty($quizobj->get_finalquiz()->showblocks)) {
...
$attempt = finalquiz_prepare_and_start_new_attempt($quizobj, $attemptnumber, $lastattempt);
