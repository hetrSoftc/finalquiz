 ...
 * @since     Moodle 2.0
 */
class moodle_finalquiz_exception extends moodle_exception {
    public function __construct($quizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $quizobj->view_url();
        }
        parent::__construct($errorcode, 'finalquiz', $link, $a, $debuginfo);
    }
}
...
class finalquiz {
    ...
    /** @var finalquiz_access_manager the access manager for this quiz. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/finalquiz:preview. */
     ...
     *
     * @param object $quiz the row from the finalquiz table.
     * @param object $cm the course_module object for this finalquiz.
     ...
     *
     * @param int $quizid the the finalquiz id.
     * @param int $userid the the userid.
     * @return finalquiz the new finalquiz object
     */
    public static function create($quizid, $userid = null) {
        global $DB;

        $quiz = finalquiz_access_manager::load_finalquiz_and_settings($quizid);
        $course = $DB->get_record('course', array('id' => $quiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('finalquiz', $quiz->id, $course->id, false, MUST_EXIST);

        // Update quiz with override information.
        if ($userid) {
            $quiz = finalquiz_update_effective_access($quiz, $userid);
        }

        return new finalquiz($quiz, $cm, $course);
    }

    /**
     * Create a {@link quiz_attempt} for an attempt at this quiz.
     * @param object $attemptdata row from the finalquiz_attempts table.
     * @return finalquiz_attempt the new quiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new finalquiz_attempt($attemptdata, $this->quiz, $this->cm, $this->course);
    }
    ...
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{finalquiz_slots} slot ON slot.quizid = :quizid AND q.id = slot.questionid',
    ...
    /**
     * Get an instance of the {@link \mod_finalquiz\structure} class for this quiz.
     * @return \mod_finalquiz\structure describes the questions in the quiz.
     */
    public function get_structure() {
        return \mod_finalquiz\structure::create_for_finalquiz($this);
    }
    ...
    public function get_finalquizid() {
        return $this->quiz->id;
    }
    ...
    public function get_finalquiz_name() {
    ...
    /** @return int the number of attempts allowed at this final quiz (0 = infinite). */
    ...
    /** @return object the module context for this final quiz. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the final quiz,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/finalquiz:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this final quiz.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'finalquiz', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this final quiz.
     * @return array 0, 1, 2, ... => final quiz_sections row from the database.
     */
    ...
    /**
     * Return finalquiz_access_manager and instance of the finalquiz_access_manager class
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new quiz_access_manager($this, $timenow,
                    has_capability('mod/finalquiz:ignoretimelimits', $this->context, null, false));
        }
    ...
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/finalquiz/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this finalquiz's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/finalquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/finalquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
    ...
    /**
     * @return string the URL of this final quiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/finalquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/finalquiz/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/finalquiz/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by finalquiz access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_finalquiz_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_finalquiz_display_options::DURING ||
                $when == mod_finalquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_finalquiz_display_options::LATER_WHILE_OPEN && $this->quiz->timeclose &&
                $this->quiz->reviewattempt & mod_finalquiz_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'finalquiz',
                    userdate($this->quiz->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'finalquiz');
        }
    }
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_finalquiz_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this finalquiz.
     *
     * @param  boolean $includepotential if the finalquiz include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the finalquiz may content
...
/**
 * This class extends the finalquiz class to hold data about the state of a particular attempt,
 * in addition to the data about the finalquiz.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class finalquiz_attempt {
    ...
    /** @var int maximum number of slots in the finalquiz for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var finalquiz object containing the quiz settings. */
    protected $quizobj;

    /** @var stdClass the finalquiz_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this finalquiz attempt. */
    protected $quba;
    ...
    /** @var array of finalquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_finalquiz_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the finalquiz_attempts table.
    ...
    public function __construct($attempt, $quiz, $cm, $course, $loadquestions = true) {
        ...
        $this->quizobj = new finalquiz($quiz, $cm, $course);
     ...
     * @param array $conditions passed to $DB->get_record('finalfinalquiz_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('finalfinalquiz_attempts', $conditions, '*', MUST_EXIST);
        $quiz = finalquiz_access_manager::load_finalquiz_and_settings($attempt->quiz);
        $course = $DB->get_record('course', array('id' => $quiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('finalquiz', $quiz->id, $course->id, false, MUST_EXIST);

        // Update quiz with override information.
        $quiz = finalquiz_update_effective_access($quiz, $attempt->userid);

        return new finalquiz_attempt($attempt, $quiz, $cm, $course);
    }

    /**
     * Static function to create a new finalquiz_attempt object given an attemptid.
     *
     * @param int $attemptid the attempt id.
     * @return finalquiz_attempt the new finalquiz_attempt object
     */
    ...
    /**
     * Static function to create a new finalquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return finalquiz_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'finalquiz');
                }
    ...
    // Simple getters ==========================================================
    public function get_finalquiz() {
        return $this->quizobj->get_finalquiz();
    }

    public function get_finalquizobj() {
        return $this->quizobj;
    }
    ...
    /** @return int the quiz id. */
    public function get_finalquizid() {
        return $this->quizobj->get_finalquizid();
    }

    /** @return string the name of this quiz. */
    public function get_finalquiz_name() {
        return $this->quizobj->get_finalquiz_name();
    }

    /** @return int the finalquiz navigation method. */
    public function get_navigation_method() {
    ...
    /** @return object the row from the finalfinalquiz_attempts table. */
    ...
    /** @return string one of the finalquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/finalquiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the finalquiz in a non-trivial way?
     ...
     * To see how this works, try setting a final quiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the final quiz
     ...
     *
     * @param question_display_options $options the display options for this final quiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return finalquiz_feedback_for_grade($grade, $this->get_finalquiz(),
                $this->quizobj->get_context());
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the final quiz context.
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the final quiz context.
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_finalquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/finalquiz:attempt';
        } else {
            $capability = 'mod/finalquiz:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a final quiz
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the final quiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/finalquiz:viewreports') ||
                $this->has_capability('mod/finalquiz:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case FINALQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case FINALQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return finalquiz_attempt_state($this->get_finalquiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_finalquiz_display_options for this final quiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = finalquiz_get_review_options($this->get_finalquiz(),
            ...
            $options = mod_finalquiz_display_options::make_from_finalquiz($this->get_finalquiz(),
                    mod_finalquiz_display_options::DURING);
            $options->flags = finalquiz_get_flag_option($this->attempt, $this->quizobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the final quiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the final quiz, or for the
     * whole final quiz.
     *
    ...
    public function get_question_usage() {
        if (!PHPUNIT_TEST) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the finalquiz_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != FINALQUIZ_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_finalquiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the final quiz where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the final quiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the final quiz.
     */
    public function get_question_mark($slot) {
        return finalquiz_format_question_grade($this->get_finalquiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->quizobj->get_finalquiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->quizobj->get_finalquiz()->timelimit;
        }
        if ($this->quizobj->get_finalquiz()->timeclose) {
            $deadlines[] = $this->quizobj->get_finalquiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->quizobj->get_finalquiz()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string finalquiz view url.
     */
    public function view_url() {
    ...
    /**
     * @return string the URL of this final quiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/finalquiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/finalquiz/processattempt.php');
    }
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'finalquiz'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_finalquiz_renderer $renderer the final quiz renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_finalquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_finalquiz-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_finalquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'finalquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'finalquiz');
        ...
        $placeholderqa->start($this->get_finalquiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this final quiz attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_finalquiz_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, finalquiz_attempt_nav_panel or finalquiz_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole final quiz on one page. (Used by review.php)
     * @return finalquiz_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_finalquiz_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_finalquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_finalquiz_navblock_title';
        $bc->title = html_writer::span(get_string('finalquiznavigation', 'finalquiz'), '', array('id' => 'mod_finalquiz_navblock_title'));
     ...
     *
     * The {@link mod_finalquiz_links_to_other_attempts} object returned contains an
    ...
    public function links_to_other_attempts(moodle_url $url) {
        ...
        $links = new mod_finalquiz_links_to_other_attempts();
     ...
     * @return mod_finalquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->quizobj->get_finalquiz()->graceperiod;
        ...
        switch ($this->quizobj->get_finalquiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('finalfinalquiz_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            finalquiz_save_best_grade($this->get_finalquiz(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_finalquiz\question\qubaids_for_users_attempts(
                $this->get_finalquizid(), $this->get_userid());
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = finalquiz_retrieve_slot_tag_ids($this->slots[$slot]->id);
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            finalquiz_save_best_grade($this->get_finalquiz(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_finalquiz\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('finalfinalquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_finalquiz\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        finalquiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_finalquiz\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $quizrecord = $this->get_finalquiz();
        $params = array(
            'context' => $this->get_finalquizobj()->get_context(),
        ...
        $event->add_record_snapshot('finalquiz', $this->get_finalquiz());
        $event->add_record_snapshot('finalfinalquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
     ...
     *
     * @param string $script. Used in the URL like /mod/finalquiz/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/finalquiz/' . $script . '.php' . $fragment,
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - FINALQUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('finalquiz', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_finalquiz()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('finalquiz', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_finalquiz()->graceperiod + $graceperiodmin) {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == FINALQUIZ_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('finalfinalquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'quizid' => $this->get_finalquizid()
            )
        );
        $event = \mod_finalquiz\event\attempt_viewed::create($params);
        $event->add_record_snapshot('finalfinalquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    public function fire_attempt_summary_viewed_event() {
        ...
        $event = \mod_finalquiz\event\attempt_summary_viewed::create($params);
        $event->add_record_snapshot('finalfinalquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    public function fire_attempt_reviewed_event() {
        ...
        $event = \mod_finalquiz\event\attempt_reviewed::create($params);
        $event->add_record_snapshot('finalfinalquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
 ...
 * @since      Moodle 2.9
 */
class finalquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class finalquiz_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class finalquiz_nav_panel_base {
    /** @var finalquiz_attempt */
    ...
    public function __construct(finalquiz_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the final quiz navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new finalquiz_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new finalquiz_nav_question_button();
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'finalquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'finalquiz');
        } else {
            return get_string('viewed', 'finalquiz');
        }
    }

    public function render_before_button_bits(mod_finalquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_finalquiz_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_finalquiz()->showuserpicture == FINALQUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_finalquiz()->showuserpicture == FINALQUIZ_SHOWIMAGE_LARGE) {
...
/**
 * Specialisation of {@link finalquiz_nav_panel_base} for the attempt quiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class finalquiz_attempt_nav_panel extends finalquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_finalquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'finalquiz'),
                array('id' => 'quiznojswarning'));
    }

    public function render_end_bits(mod_finalquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'finalquiz'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link finalquiz_nav_panel_base} for the review quiz page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class finalquiz_review_nav_panel extends finalquiz_nav_panel_base {
    public function get_question_url($slot) {
        ...
        if ($this->attemptobj->get_num_pages() > 1) {
            if ($this->showall) {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, false),
                        get_string('showeachpage', 'finalquiz'));
            } else {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, true),
                        get_string('showall', 'finalquiz'));
            }
        }
